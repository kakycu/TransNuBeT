...
if (!permiso_puede('empleados', 'ver')) {
    permiso_denegar_acceso('Empleados');
}

// Capacidades del rol actual (controlan la visibilidad de botones en la UI)
$puede_crear_empleados    = permiso_puede('empleados', 'crear');
$puede_editar_empleados   = permiso_puede('empleados', 'editar');
$puede_eliminar_empleados = permiso_puede('empleados', 'eliminar');
$puede_exportar_empleados = permiso_puede('empleados', 'exportar');
$puede_gestionar_empleados = $puede_crear_empleados || $puede_editar_empleados || $puede_eliminar_empleados;
...
function exportar_anexo_14($pdo) {
    ...
    $nombre_empresa = $config['nombre_empresa'] ?? COMPANY_NAME;
    $jefe_proyecto = $config['jefe_proyecto'] ?? JEFE_PROYECTO;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // CONTROL DE ACCESO POR PERMISOS (seguridad servidor)
    $permisos_por_accion = [
        'crear'    => 'crear',
        'editar'   => 'editar',
        'eliminar' => 'eliminar',
        'verificar_expediente' => 'ver',
    ];
    if (isset($permisos_por_accion[$action]) && !permiso_puede('empleados', $permisos_por_accion[$action])) {
...
// Abrir el modal en modo "Nuevo Empleado"
$abrir_modal_nuevo = $puede_crear_empleados && isset($_GET['nuevo']) && $_GET['nuevo'] !== '0' && $_GET['nuevo'] !== '';
...
<style>
        * { margin:0; padding:0; box-sizing: border-box; }
        ...
        .text-muted, .text-secondary { color: rgba(255, 255, 255, 0.65) !important; }
        ...
        /* Buttons Windows 11 */
        .btn-win {
            background: rgba(255, 255, 255, 0.08); border: 0.0625rem solid rgba(255, 255, 255, 0.1); padding:0.5rem 1rem;
            border-radius: 0.625rem; color: white; font-size:0.85rem; transition: all 0.2s; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap:0.5rem;
        }
        ...
        .form-select, .form-control { background: var(--panel); border: 0.0625rem solid rgba(255, 255, 255, 0.1); border-radius: 0.625rem; color: var(--txt); padding:0.5rem 0.75rem; }
        .form-select:focus, .form-control:focus { background: var(--panel); border-color: #60a5fa; outline: none; box-shadow: 0 0 0 0.125rem rgba(96, 165, 250, 0.2); color: var(--txt); }
        
        /* Controles deshabilitados (modo solo lectura) mantienen el tema oscuro */
        ...
        /* Cards colapsables */
        .card-collapse-header { cursor: pointer; user-select: none; }
        .card-collapse-header:hover .card-collapse-title { color: #60a5fa !important; }
        .card-collapse-title { transition: color 0.2s ease; }
        .card-collapse-chevron { transition: transform 0.25s ease, color 0.2s ease; color: rgba(255, 255, 255, 0.4); font-size:0.75rem; }
        .card-collapse-header[aria-expanded="true"] .card-collapse-chevron { transform: rotate(180deg); color: #60a5fa; }
        .card-collapse-header[aria-expanded="true"] .card-collapse-title { color: #60a5fa !important; }

        /* Badge BAJA parpadeante */
        @keyframes blinkBaja { 0%, 49% { opacity: 1; } 50%, 100% { opacity: 0.15; } }
        .badge-baja {
            display: none; align-items: center; gap:0.375rem;
            color: #ef4444; font-weight: 800; font-size:1rem; letter-spacing:0.125rem;
            text-shadow: 0 0 0.625rem rgba(239, 68, 68, 0.7);
            animation: blinkBaja 1s step-start infinite;
        ...
        .modal-content-win { 
            ...
            color: var(--txt); 
            box-shadow: 0 0 1.875rem rgba(96, 165, 250, 0.3);
        }
        
        @keyframes modalGlow {
            0% { box-shadow: 0 0 0 0 rgba(96, 165, 250, 0); border-color: rgba(96, 165, 250, 0.3); }
            100% { box-shadow: 0 0 1.875rem 0.3125rem rgba(96, 165, 250, 0.4); border-color: #60a5fa; }
        }
...
.modal-empleado-nombre:hover {
    ...
    border-color: #60a5fa;
    box-shadow: 0 0 0.75rem rgba(96, 165, 250, 0.3);
}
...
.btn-nav:hover:not(:disabled) {
    ...
    color: white;
    box-shadow: 0 0.125rem 0.5rem rgba(96, 165, 250, 0.4);
}
...
.nav-selector select:focus {
    outline: none;
    border-color: #60a5fa;
    box-shadow: 0 0 0 0.125rem rgba(96, 165, 250, 0.3);
}
...
@media (max-width: 480px) {
    .nav-selector {
        ...
        .modal-body-win { padding:1rem 1.25rem !important; }
        ...
        .modal-body-win .avatar-preview { width:7.5rem !important; height:7.5rem !important; }
        .modal-body-win .avatar-upload { max-width:10rem !important; margin-bottom:0 !important; }
        .modal-body-win .text-center.mb-4 { margin-bottom:0.5rem !important; }
        .modal-body-win .info-card { padding:0.375rem 0.625rem !important; margin-bottom:0.375rem !important; }
        .modal-body-win .info-card-label { font-size:0.6rem !important; }
        .modal-body-win .info-card-value { font-size:0.75rem !important; }
        .modal-body-win .info-scroll { max-height:calc(80vh - 9.375rem) !important; }
        .modal-footer-win .btn-win { padding:0.25rem 0.75rem !important; font-size:0.75rem !important; }
        ...
        .avatar-preview img[src*="base64"]:hover {
            ...
            filter: drop-shadow(0 0 0.5rem rgba(96, 165, 250, 0.5));
        }
        ...
        .avatar-placeholder i { font-size:3rem; }
        ...
        .edit-overlay i { font-size:0.75rem; }
        ...
        .data-table-wrapper table { 
            min-width:109.375rem !important;
            width:100%; 
        ...
        .data-table-wrapper th { 
            ...
            box-shadow: inset 0 -0.125rem 0 rgba(255, 255, 255, 0.12), inset 0 0.0625rem 0 rgba(255, 255, 255, 0.05);
            font-weight: 600; 
            font-size:0.8rem; 
            text-transform: uppercase; 
            letter-spacing:0.0312rem; 
        ...
        .dataTables_wrapper .dataTables_length select, .dataTables_wrapper .dataTables_filter input { 
            ...
            border-radius: 0.5rem !important; color: #ffffff !important; padding:0.375rem 0.75rem !important; 
        }
        .dataTables_wrapper .dataTables_filter input:focus { border-color: #60a5fa !important; outline: none !important; box-shadow: 0 0 0 0.125rem rgba(96, 165, 250, 0.2) !important; }
        ...
        .dataTables_wrapper .dataTables_paginate .paginate_button.disabled { color: rgba(255, 255, 255, 0.4) !important; background: var(--panel-2) !important; border-color: rgba(255, 255, 255, 0.08) !important; cursor: not-allowed !important; }
        ...
        table.dataTable tbody tr.even { background: rgba(var(--accent-rgb), 0.03) !important; }
        table.dataTable tbody tr:hover { background: rgba(var(--accent-rgb), 0.15) !important; }
        table.dataTable tbody tr:hover td { background: transparent !important; }
        table.dataTable tbody tr:hover td:first-child { border-left: 0.1875rem solid var(--accent); }
        [data-theme="light"] table.dataTable tbody tr.odd { background: rgba(0, 0, 0, 0.015) !important; }
        [data-theme="light"] table.dataTable tbody tr.even { background: rgba(var(--accent-rgb), 0.04) !important; }
        [data-theme="light"] table.dataTable tbody tr:hover { background: rgba(var(--accent-rgb), 0.18) !important; }
        ...
        .data-table-wrapper table.dataTable tbody td.text-success { color: var(--color-success-soft) !important; }
        
        /* Alertas Vacaciones */
        #empleadosTable tbody tr.vacaciones-excedidas td { background-color: rgba(220, 53, 69, 0.12) !important; color: #ff8888 !important; }
        #empleadosTable tbody tr.vacaciones-excedidas td:first-child { border-left: 0.1875rem solid #dc3545; }
        .alerta-vacaciones-flotante { background: rgba(220, 53, 69, 0.15); border-left: 0.25rem solid #ff4444; border-radius: 0.5rem; padding:0.75rem 0.9375rem; margin-bottom:0.9375rem; display: flex; align-items: center; justify-content: space-between; }
        ...
        /* Cuenta bancaria */
        #digitosCuenta.valid { color: var(--color-success); font-weight: bold; }
        ...
        .img-container {
            ...
            background-position: 0 0, 0 0.625rem, 0.625rem -0.625rem, -0.625rem 0rem;
            min-height:25rem;
            border-radius: 0.5rem;
            overflow: hidden;
        }
        ...
        .tooltip-inner {
            ...
            color: #ffffff !important;
            border: 0.0625rem solid #3b82f6 !important;
        ...
        #empleadosTable tbody td:nth-child(2) img:hover {
            transform: scale(1.8);
            border-color: #60a5fa;
            box-shadow: 0 0 0.9375rem rgba(96, 165, 250, 0.6);
            z-index: 100;
            position: relative;
        }
        ...
        .foto-modal-win {
            border-radius: 1.25rem !important;
            border: 0.0625rem solid rgba(96, 165, 250, 0.3) !important;
            box-shadow: 0 0 1.875rem rgba(0, 0, 0, 0.5) !important;
            backdrop-filter: blur(0.625rem) !important;
        }
        
        /* DataTables extras */
        .dt-buttons { display: none !important; gap:0.5rem; flex-wrap: wrap; }
        ...
        @media (max-width: 768px) { 
            ...
            .dataTables_length { width:100% !important; margin-bottom:0.625rem !important; }
            .info-card-label { font-size:0.55rem; }
            .info-card-value { font-size:0.65rem; }
            .info-card { padding:0.25rem 0.375rem; margin-bottom:0.25rem; }
 .modal-empleado-nombre {
        font-size:0.9rem;
    ...
    .modal-empleado-nombre-wrapper {
        ...
        @media (max-width: 480px) {
            ...
            .nav-controls { min-width:8.125rem; }
        }
/* Card tipo Solapín */
.solapin-card {
...
.solapin-card:hover {
    ...
    border-color: #60a5fa;
    box-shadow: 0 0.375rem 1rem rgba(96, 165, 250, 0.2);
}
...
.solapin-nombre {
    font-size:0.9rem;
    color: #ffffff;
    border-bottom: 0.0625rem dashed rgba(96, 165, 250, 0.3);
...
.donut-custom { position: relative; width:10.625rem; height:10.625rem; border-radius: 50%; margin:0 auto; }
.donut-hole {
    position: absolute; top:1.125rem; left:1.125rem; width:8.375rem; height:8.375rem; border-radius: 50%;
    background: #14141c; display: flex; flex-direction: column; align-items: center; justify-content: center;
}

/* ===== Select2: filtro "Trabajador" con búsqueda ===== */
...
html .badge.badge-estado-activo {
    background: rgba(16, 185, 129, 0.18) !important;
    color: #10b981 !important;
    border: 0.0625rem solid rgba(16, 185, 129, 0.45);
}
html .badge.badge-estado-inactivo {
    background: rgba(220, 38, 38, 0.15) !important;
    color: #ef4444 !important;
    border: 0.0625rem solid rgba(220, 38, 38, 0.45);
}
html[data-theme="light"] .badge.badge-estado-activo {
    background: rgba(4, 120, 87, 0.12) !important;
    color: #047857 !important;
    border-color: rgba(4, 120, 87, 0.4);
}
html[data-theme="light"] .badge.badge-estado-inactivo {
    ...
    <div class="win-topbar fade-in-up">
        <div class="d-flex align-items-center gap-3">
            ...
            <div class="page-title">
                ...
                <p><i class="fas fa-users me-1"></i> Registro y control del personal</p>
            </div>
        </div>
        
<div class="user-menu">
    <?php if ($puede_crear_empleados): ?>
    <button class="btn-win btn-win-primary" data-bs-toggle="modal" data-bs-target="#empleadoModal" onclick="clearFormAndStartNew()" title="Nuevo Empleado" data-tooltip="Nuevo Empleado" data-tooltip-theme="success">
    ...
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-win dropdown-menu-end">
            ...
            <li><h6 class="dropdown-header text-light px-3 py-1">Credenciales y Cumpleaños</h6></li>
            ...
            <li><hr class="dropdown-divider"></li>
            <li><a class="dropdown-item" href="#" id="menuExportCumpleCompleto"><i class="fas fa-birthday-cake text-warning me-2"></i> Exportar Listado Completo de Cumpleaños</a></li>
            <li><a class="dropdown-item" href="#" id="menuExportCumple30"><i class="fas fa-calendar-day text-success me-2"></i> Exportar Cumpleaños Próximos 30 días</a></li>
            <li><hr class="dropdown-divider"></li>
    ...
    <?php
    // Cumpleaños de HOY (alerta tipo tarjeta, como las del sistema)
    $cumple_hoy = array_values(array_filter($cumpleaneros, function ($c) { return $c['dias'] == 0; }));
    if (count($cumple_hoy) > 0):
        $links_hoy = array_map(function ($c) {
            return '<a href="#" onclick="event.preventDefault(); verFotoGrande(\'' . addslashes($c['foto']) . '\', \'' . addslashes($c['nombre']) . '\', ' . $c['id'] . ');" style="color:inherit; text-decoration:underline; cursor:pointer;">' . htmlspecialchars($c['nombre']) . '</a>';
        }, $cumple_hoy);
    ?>
    ...
    <div class="row g-3 mb-4 fade-in-up">
        <div class="col-12">
            <div class="alert alert-win alert-dismissible fade show"
                 style="background: rgba(var(--color-success-rgb), 0.15);
                        border: 0.0625rem solid rgba(var(--color-success-rgb), 0.35);
                        color: var(--color-success); border-radius: 0.75rem; padding:0.9375rem 1.25rem;">
                <i class="fas fa-birthday-cake me-2"></i>
                <?php echo count($cumple_hoy); ?> empleado(s) cumplen años HOY: <?php echo implode(', ', $links_hoy); ?>
    ...
    <div class="row g-3 mb-4 fade-in-up" style="animation-delay: 0.06s;">
        <div class="col-md-6">
            <div class="glass-card p-4 h-100">
                <div class="card-collapse-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#collapseGenero" aria-expanded="false" aria-controls="collapseGenero">
                    <h6 class="mb-0 text-light card-collapse-title"><i class="fas fa-venus-mars me-2" style="color: #a78bfa;"></i> Distribución por Género</h6>
                    <div class="d-flex align-items-center gap-2">
                        <button type="button" class="btn-win btn-win-sm" onclick="event.stopPropagation(); exportarGeneroPNG()" title="Exportar gráfico a PNG" data-tooltip="Exportar gráfico a PNG" data-tooltip-theme="info"><i class="fas fa-download me-1"></i> PNG</button>
        ...
        <div class="col-md-6">
            <div class="glass-card p-4 h-100">
                <div class="card-collapse-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#collapseEdad" aria-expanded="false" aria-controls="collapseEdad">
                    <h6 class="mb-0 text-light card-collapse-title"><i class="fas fa-chart-bar me-2" style="color: #60a5fa;"></i> Rangos de Edad</h6>
                    <div class="d-flex align-items-center gap-2">
                        <button type="button" class="btn-win btn-win-sm" onclick="event.stopPropagation(); exportarRangosEdadPNG()" title="Exportar gráfico a PNG" data-tooltip="Exportar gráfico a PNG" data-tooltip-theme="info"><i class="fas fa-download me-1"></i> PNG</button>
                        <i class="fas fa-chevron-down card-collapse-chevron"></i>
                    </div>
                </div>
                <div class="collapse" id="collapseEdad">
    ...
    <div class="col-12">
        <div class="glass-card h-100">
            <div class="p-3 border-bottom border-white-10 d-flex justify-content-between align-items-center card-collapse-header" data-bs-toggle="collapse" data-bs-target="#collapseCumples" aria-expanded="false" aria-controls="collapseCumples">
                <h6 class="mb-0 fw-semibold text-light card-collapse-title">
                    <i class="fas fa-birthday-cake fa-fw me-2" style="font-size:1.05rem;"></i>Cumpleaños de los Empleados (NO INCLUYE BAJAS)
                </h6>
                <div class="d-flex align-items-center gap-2">
                    <div class="dropdown">
                        <button class="btn-win btn-win-sm" type="button" data-bs-toggle="dropdown" aria-expanded="false" onclick="event.stopPropagation();" style="padding:0.3125rem 0.6875rem; font-size:0.72rem; background: rgba(255,255,255,0.08); border: 0.0625rem solid rgba(255,255,255,0.12);">
                            <i class="fas fa-download me-1"></i><i class="fas fa-chevron-down ms-1" style="color: rgba(255,255,255,0.6); font-size:0.6rem;"></i>
                        </button>
                        <ul class="dropdown-menu dropdown-menu-win dropdown-menu-end">
                            <li><h6 class="dropdown-header text-light px-3 py-1">Exportar</h6></li>
                            <li><a class="dropdown-item" href="#" id="cardCumpleExportCompleto"><i class="fas fa-birthday-cake text-warning me-2"></i> Exportar Listado Completo</a></li>
                            <li><a class="dropdown-item" href="#" id="cardCumpleExport30"><i class="fas fa-calendar-day text-success me-2"></i> Exportar Próximos 30 días</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><h6 class="dropdown-header text-light px-3 py-1">Imprimir</h6></li>
                            <li><a class="dropdown-item" href="#" id="cardCumplePrintCompleto"><i class="fas fa-print text-danger me-2"></i> Imprimir Listado Completo</a></li>
                            <li><a class="dropdown-item" href="#" id="cardCumplePrint30"><i class="fas fa-print text-warning me-2"></i> Imprimir Próximos 30 días</a></li>
                        </ul>
                    </div>
            ...
            <div class="collapse" id="collapseCumples">
            <div class="p-3">
                <div class="row g-3">
                    <div class="col-lg-5">
                    ...
                    <div class="col-lg-7">
                        <div class="glass-card p-2" style="background: rgba(0,0,0,0.2); border: 0.0625rem solid rgba(255,255,255,0.05);">
                            <h6 class="text-center mb-2" style="color: rgba(255,255,255,0.7); font-size:0.8rem; letter-spacing:0.0312rem;">
    ...
    <div class="col-12">
    <div class="glass-card h-100">
        <div class="p-3 border-bottom border-white-10 d-flex justify-content-between align-items-center card-collapse-header" data-bs-toggle="collapse" data-bs-target="#collapseFiltros" aria-expanded="false" aria-controls="collapseFiltros">
            <h6 class="mb-0 fw-semibold text-light card-collapse-title"><i class="fas fa-filter me-2"></i> Filtros de Búsqueda</h6>
            <i class="fas fa-chevron-down card-collapse-chevron"></i>
        </div>
        ...
        <div class="p-4">
            <div class="row g-3 align-items-end">
                <div class="col-md-3">
                    ...
                    <select id="filtroEmpleado" class="form-select" title="Filtrar por empleado" data-tooltip="Filtrar por empleado" data-tooltip-theme="secondary">
                ...
                <div class="col-md-3">
                    ...
                    <select id="filtroTipoContrato" class="form-select" title="Filtrar por tipo de contrato" data-tooltip="Filtrar por tipo de contrato" data-tooltip-theme="secondary">
                ...
                <div class="col-md-3">
                    ...
                    <select id="filtroPagoVacaciones" class="form-select" title="Filtrar por pago de vacaciones" data-tooltip="Filtrar por pago de vacaciones" data-tooltip-theme="secondary">
                ...
                <div class="col-md-3">
                    ...
                    <select id="filtroCuentaBancaria" class="form-select" title="Filtrar por cuenta bancaria" data-tooltip="Filtrar por cuenta bancaria" data-tooltip-theme="secondary">
                        <option value="">-- Todos --</option>
...
<div class="col-md-3">
    ...
    <select id="filtroFoto" class="form-select" title="Filtrar por estado de foto" data-tooltip="Filtrar por estado de foto" data-tooltip-theme="secondary">
        <option value="">-- Todos --</option>
...
<div class="col-md-3">
    ...
    <select id="filtroEstadoLaboral" class="form-select" title="Filtrar por estado laboral" data-tooltip="Filtrar por estado laboral" data-tooltip-theme="secondary">
        <option value="">-- Todos --</option>
...
<div class="col-md-3">
    <label class="form-label"><i class="fas fa-hourglass-end text-muted me-1"></i> Días de Vacaciones</label>
    <select id="filtroVacacionesExcedidas" class="form-select" title="Filtrar por días de vacaciones acumuladas" data-tooltip="Filtrar por días de vacaciones acumuladas" data-tooltip-theme="secondary">
        <option value="">-- Todos --</option>
        <option value="0">⚪ 0 días</option>
        <option value="menor1">🔵 &lt; De un día</option>
        <option value="1-5">🟢 De 1 a 5 días</option>
        <option value="5-10">🟡 De 5 a 10 días</option>
        <option value="10-15">🟠 De 10 a 15 días</option>
        <option value="15-20">🔴 De 15 a 20 días</option>
...
<td class="text-center" data-tiene-foto="<?php echo (!empty($emp['foto_ruta']) && file_exists(__DIR__ . '/../' . $emp['foto_ruta'])) ? '1' : '0'; ?>">
    <?php if (!empty($emp['foto_ruta']) && file_exists(__DIR__ . '/../' . $emp['foto_ruta'])): ?>
        <img src="<?php echo '../' . $emp['foto_ruta']; ?>" 
             style="width:2.1875rem; height:2.1875rem; object-fit: cover; border-radius: 50%; cursor: pointer;" 
    ...
    <?php else: ?>
        <img src="<?php echo $logo_base64; ?>" 
             style="width:2.1875rem; height:2.1875rem; object-fit: contain; border-radius: 50%; cursor: pointer; background: rgba(96, 165, 250, 0.1); padding:0.3125rem;" 
    ...
    <?php endif; ?>
                ...
                <tfoot>
                    <tr>
                        <td colspan="9" class="text-end" style="color: #60a5fa; padding:0.75rem;">
                            TOTALES:
                        </td>
                        <td class="text-end" style="color: var(--color-success-soft); padding:0.75rem; background: rgba(var(--color-success-rgb), 0.1);">
                        ...
                        <td class="text-center">-</td>
                        <td class="text-end">-</td>
                        <td class="text-end" style="color: var(--color-success-soft); padding:0.75rem;">
                            <?php echo formatearMoneda((float)array_sum(array_column($empleados, 'valor_vacaciones') ?: [0])); ?>
                        </td>
                        <td class="text-end" style="color: #fbbf24; padding:0.75rem;">
                            <?php 
                            $total_pagar = 0;
                            foreach ($empleados as $emp) {
...
<script>
// ==========================================
// EXPORTAR VISUALIZACIONES A PNG (con colores del tema actual)
// ==========================================
function tmbFecha() {
    var a = new Date();
    var anio = a.getFullYear();
    var mesTxt = a.toLocaleString('es-ES', { month: 'long' });
    var dia = a.getDate();
    return { completo: dia + ' de ' + mesTxt + ' de ' + anio, nombre: anio + '-' + String(a.getMonth() + 1).padStart(2, '0') + '-' + String(dia).padStart(2, '0') };
}
function descargarPNG(tempCanvas, nombreBase) {
    var f = tmbFecha();
    var link = document.createElement('a');
    link.download = nombreBase + '_' + f.nombre + '.png';
    link.href = tempCanvas.toDataURL('image/png', 1.0);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}
function crearLienzoExport(anchoReal, altoReal) {
    var escala = 2;
    ...
    return { canvas: tempCanvas, ctx: ctx, ancho: anchoReal, alto: altoReal, tituloAlt: alturaTitulo / escala };
}
function pintarEncabezadoExport(ctx, ancho, titulo, tituloAlt) {
    ...
    ctx.fillText('📅 Generado el ' + tmbFecha().completo, ancho / 2, 48);
...
function exportarGeneroPNG() {
    ...
    var pctH = <?php echo (float)$pct_hombres; ?>;
    var pctM = <?php echo (float)$pct_mujeres; ?>;
    var temaActual = document.documentElement.getAttribute('data-theme');
    var oscuro = temaActual !== 'light' && temaActual !== 'orgullo';
    var l = crearLienzoExport(720, 440);
    var ctx = l.ctx;
    pintarEncabezadoExport(ctx, l.ancho, 'Distribución por Género', l.tituloAlt);
    ...
    ctx.fillStyle = oscuro ? '#14141c' : (temaActual === 'orgullo' ? '#f4eefb' : '#ffffff');
    ctx.fill();
    ctx.textAlign = 'center';
    ...
    descargarPNG(l.canvas, 'distribucion_genero');
}

function exportarRangosEdadPNG() {
    var rangos = <?php echo json_encode($rangos_edad, JSON_UNESCAPED_UNICODE); ?>;
    var totalActivos = <?php echo (int)$total_activos; ?>;
    var labels = Object.keys(rangos);
    var l = crearLienzoExport(720, 40 + labels.length * 70);
    var ctx = l.ctx;
    pintarEncabezadoExport(ctx, l.ancho, 'Rangos de Edad', l.tituloAlt);
    var colW = (l.ancho - 70) / 2;
    var startY = 150;
    var rowH = 90;
    var gradBar = ctx.createLinearGradient(0, 0, colW, 0);
    gradBar.addColorStop(0, '#60a5fa');
    gradBar.addColorStop(1, '#a78bfa');
    for (var j = 0; j < labels.length; j++) {
        var col = j % 2;
        var fila = Math.floor(j / 2);
        var x = 35 + col * colW;
        var y = startY + fila * rowH;
        var val = rangos[labels[j]];
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = window.exportColor().muted;
        ctx.font = '0.875rem Inter, Segoe UI, sans-serif';
        ctx.fillText(labels[j] + ' años', x, y);
        ctx.textAlign = 'right';
        ctx.fillStyle = window.exportColor().titulo;
        ctx.font = 'bold 0.9375rem Inter, Segoe UI, sans-serif';
        ctx.fillText(String(val), x + colW - 4, y);
        var py = y + 12;
        ctx.fillStyle = window.exportColor().zebra;
        ctx.fillRect(x, py, colW, 16);
        var pct = totalActivos > 0 ? val / totalActivos : 0;
        ctx.fillStyle = gradBar;
        ctx.fillRect(x, py, Math.max(8, colW * Math.min(pct, 1)), 16);
    }
    descargarPNG(l.canvas, 'rangos_edad');
}
</script>
...
<div class="modal-header modal-header-win py-2">
    ...
    <div class="d-flex align-items-center gap-2 flex-shrink-0">
        ...
        <span id="badgeSoloLectura" style="display: none; align-items: center; gap:0.25rem; background: rgba(251, 191, 36, 0.15); color: #fbbf24; border: 0.0625rem solid rgba(251, 191, 36, 0.4); padding:0.125rem 0.5rem; border-radius: 1.25rem; font-size:0.65rem; font-weight: 600; white-space: nowrap;">
...
<div class="col-md-3">
    ...
    <div class="text-center mb-2">
        <div class="avatar-preview mb-1" id="avatarPreviewContainer" onclick="abrirEditorFotoDesdePreview()" style="width:6.25rem; height:6.25rem;">
            <div id="fotoPlaceholder" class="avatar-placeholder"><i class="fas fa-camera" style="font-size:2rem;"></i></div>
            <img id="imagePreview" src="" style="display: none; width:100%; height:100%; object-fit: cover;">
            <div class="edit-overlay py-1" style="font-size:0.5625rem;"><i class="fas fa-crop-alt me-1"></i> Editar</div>
        </div>
        ...
        <div class="d-flex gap-1 justify-content-center mt-1">
            ...
            <button type="button" class="btn-win btn-win-danger btn-win-sm py-1" id="btnEliminarFoto" style="display: none; font-size:0.7rem; margin:0; flex: 1;" onclick="eliminarFotoActual()" title="Eliminar foto" data-tooltip="Eliminar foto" data-tooltip-theme="danger">
	...
	<div class="solapin-card glass-card mt-2 p-2 text-center" id="solapinCard">
		...
		<div class="solapin-foto" id="solapinFoto">
			<img id="solapinFotoImg" src="<?php echo $logo_base64; ?>" alt="Foto" style="width:3.75rem; height:3.75rem; object-fit: cover; border-radius: 50%; border: 0.125rem solid #60a5fa;">
		</div>
...
<!-- NUEVOS BOTONES DE ACCIÓN RÁPIDA DEL SOLAPÍN DENTRO DEL MODAL -->
    <div class="d-flex flex-column gap-1 mt-2 w-100" id="solapinActionsContainer" style="display: none;">
        ...
        <div class="d-flex gap-1 w-100">
            <button type="button" class="btn-win btn-win-primary btn-win-sm flex-fill justify-content-center py-1.5" id="btnModalExportarSolapin" style="font-size:0.72rem; font-weight: 600;" title="Exportar a PNG" data-tooltip="Exportar a PNG" data-tooltip-theme="info">
                <i class="fas fa-download me-1"></i> PNG
            </button>
            <button type="button" class="btn-win btn-win-primary btn-win-sm flex-fill justify-content-center py-1.5" id="btnModalExportarWordSolapin" style="font-size:0.72rem; font-weight: 600;" title="Exportar a Word" data-tooltip="Exportar a Word" data-tooltip-theme="info">
                <i class="fas fa-file-word me-1"></i> Word
            </button>
            <button type="button" class="btn-win btn-win-primary btn-win-sm flex-fill justify-content-center py-1.5" id="btnModalImprimirSolapin" style="font-size:0.72rem; font-weight: 600;" title="Imprimir Solapín" data-tooltip="Imprimir Solapín" data-tooltip-theme="info">
        ...
        <div class="row g-1">
            ...
            <div class="col-6">
                <div class="info-card py-1 px-2">
                    ...
                    <div class="info-card-value small" id="infoEscalaGrupo" style="font-size:0.6rem; color: #60a5fa; margin-top:0.25rem; word-break: break-word;">--</div>
                </div>
                    ...
                    <div class="info-card-value small text-warning" id="infoVacValor">--</div>
                        ...
                        <div class="col-md-9">
                            ...
                            <div class="glass-card mt-2 p-2">
                                <div class="card-collapse-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#collapseDatosGenerales" aria-expanded="true" aria-controls="collapseDatosGenerales">
                                    <h6 class="text-light mb-0 fs-6 card-collapse-title"><i class="fas fa-user text-muted me-1"></i>Datos Generales</h6>
                                    <i class="fas fa-chevron-down card-collapse-chevron"></i>
                                </div>
                            ...
                            <div class="row g-2">
								<div class="col-md-4">
									...
									<div id="workerIdDisplay" class="d-none" style="font-size:0.95rem; margin-top:0.125rem;">
                            ...
                            <!-- Datos Contacto -->
                            <div class="glass-card mt-2 p-2">
                                <div class="card-collapse-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#collapseContactoBanco" aria-expanded="true" aria-controls="collapseContactoBanco">
                                    <h6 class="text-light mb-0 fs-6 card-collapse-title"><i class="fas fa-address-card text-muted me-1"></i>Contacto y Banco</h6>
                                    <i class="fas fa-chevron-down card-collapse-chevron"></i>
                                </div>
                                <div class="collapse show" id="collapseContactoBanco">
                                <div class="mt-2">
                            ...
                            <div class="glass-card mt-2 p-2">
                                ...
                                <div class="collapse show" id="collapseDatosSalariales">
                                <div class="mt-2">
                                <div class="row g-2">
                                    <div class="col-md-4">
                                        <label class="form-label small mb-0">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select form-select-sm" name="tipo_contrato" id="tipo_contrato" onchange="markFormDirty()" title="Tipo de contrato laboral" data-tooltip="Tipo de contrato laboral" data-tooltip-theme="secondary">
...
<div class="col-md-8">
    ...
    <select class="form-select form-select-sm" name="cargo_id" id="cargo_id" onchange="vincularCargoPlantilla(this.value)" title="Cargo del empleado" data-tooltip="Cargo del empleado" data-tooltip-theme="secondary">
...
<div class="col-md-4">
    ...
    <select class="form-select form-select-sm" id="mes_dias_trabajados" onchange="mostrarDiasTrabajados(this.value)" title="Mes de días trabajados" data-tooltip="Mes de días trabajados" data-tooltip-theme="secondary">
    ...
    <div id="detalle_dias_trabajados" class="mt-1" style="font-size:0.75rem; color: rgba(255,255,255,0.7); display: none;">
        ...
        <span id="horas_trabajadas_texto" style="margin-left:0.625rem; color: rgba(255,255,255,0.5);">(0 horas)</span>
    </div>
</div>
							   </div>
                               </div>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content" style="background: var(--win-bg-secondary); color: var(--win-text-primary);">
            <div class="modal-header" style="border-bottom-color: var(--win-border-color);">
                ...
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" onclick="limpiarCropper()" title="Cerrar recortador" data-tooltip="Cerrar recortador" data-tooltip-theme="danger"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-8">
                        <div class="img-container" style="max-height:31.25rem; overflow: hidden; background: #333; border-radius: 0.5rem; padding:0.625rem;">
                    ...
                    <div class="col-md-4">
                        <div class="text-center mb-3">
                            ...
                            <div class="preview-container" style="width:9.375rem; height:9.375rem; margin:0 auto; overflow: hidden; border: 0.1875rem solid var(--win-accent); border-radius: 0.5rem; box-shadow: 0 0.25rem 0.75rem rgba(0,0,0,0.3); background: #2d2d2d;">
                        ...
                        <div class="crop-controls p-3" style="background: var(--win-bg-tertiary); border-radius: 0.5rem;">
                            <h6 class="mb-2">Controles</h6>
                            <div class="d-grid gap-2">
                                <button type="button" class="btn btn-outline-primary" onclick="rotarImagen(-90)" title="Rotar izquierda 90°" data-tooltip="Rotar izquierda 90°" data-tooltip-theme="primary">
                                    <i class="fas fa-undo-alt me-2"></i>Rotar izquierda 90°
                                </button>
                                <button type="button" class="btn btn-outline-primary" onclick="rotarImagen(90)" title="Rotar derecha 90°" data-tooltip="Rotar derecha 90°" data-tooltip-theme="primary">
                                    <i class="fas fa-redo-alt me-2"></i>Rotar derecha 90°
                                </button>
                                <button type="button" class="btn btn-outline-secondary" onclick="resetearCrop()" title="Resetear recorte" data-tooltip="Resetear recorte" data-tooltip-theme="primary">
                                    <i class="fas fa-sync-alt me-2"></i>Resetear recorte
                                </button>
                                <button type="button" class="btn btn-outline-info" onclick="zoomImagen(0.1)" title="Acercar" data-tooltip="Acercar" data-tooltip-theme="info">
                                    <i class="fas fa-search-plus me-2"></i>Acercar
                                </button>
                                <button type="button" class="btn btn-outline-info" onclick="zoomImagen(-0.1)" title="Alejar" data-tooltip="Alejar" data-tooltip-theme="info">
            ...
            <div class="modal-footer" style="border-top-color: var(--win-border-color);">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal" onclick="limpiarCropper()" title="Cancelar" data-tooltip="Cancelar" data-tooltip-theme="danger">
                    <i class="fas fa-times me-2"></i>Cancelar
                </button>
                <button type="button" class="btn btn-primary" onclick="aplicarRecorte()" title="Aplicar recorte" data-tooltip="Aplicar recorte" data-tooltip-theme="success">
...
<!-- MODAL EXPORTAR CUMPLEAÑOS (SIN OUTSIDE CLICK) -->
<div class="modal fade" id="exportarCumpleanosModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-content-win">
            <div class="modal-header modal-header-win">
                <h5 class="modal-title"><i class="fas fa-birthday-cake me-2" style="color: #fbbf24;"></i> Exportar Cumpleaños</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" data-tooltip="Cerrar" data-tooltip-theme="danger"></button>
            </div>
            <div class="modal-body modal-body-win p-4">
                <div id="exportarCumpleInfo" class="text-center mb-3" style="font-size:0.85rem; color: rgba(255,255,255,0.7);"></div>
                <div class="row g-2">
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('pdf')" data-tooltip="Exportar PDF" data-tooltip-theme="info">
                            <i class="fas fa-file-pdf fa-2x text-danger"></i> PDF
                        </button>
                    </div>
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('word')" data-tooltip="Exportar Word" data-tooltip-theme="info">
                            <i class="fas fa-file-word fa-2x text-primary"></i> WORD
                        </button>
                    </div>
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('excel')" data-tooltip="Exportar Excel" data-tooltip-theme="info">
                            <i class="fas fa-file-excel fa-2x text-success"></i> EXCEL
                        </button>
                    </div>
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('csv')" data-tooltip="Exportar CSV" data-tooltip-theme="info">
                            <i class="fas fa-file-csv fa-2x text-info"></i> CSV
                        </button>
                    </div>
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('txt')" data-tooltip="Exportar TXT" data-tooltip-theme="info">
                            <i class="fas fa-file-alt fa-2x text-secondary"></i> TXT
                        </button>
                    </div>
                    <div class="col-6 col-md-4">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem;" onclick="exportarCumpleanos('dbf')" data-tooltip="Exportar DBF" data-tooltip-theme="info">
                            <i class="fas fa-database fa-2x text-warning"></i> DBF
                        </button>
                    </div>
                    <div class="col-12 mt-2">
                        <button type="button" class="btn-win btn-win-sm w-100 justify-content-center py-3" style="font-size:0.78rem; flex-direction: column; gap:0.375rem; background: linear-gradient(135deg, #f59e0b, #f97316); border: none;" onclick="imprimirCumpleanos()" data-tooltip="Imprimir cumpleaños" data-tooltip-theme="info">
            ...
            <div class="modal-footer modal-footer-win">
                <button type="button" class="btn-win btn-win-sm" data-bs-dismiss="modal" data-tooltip="Cancelar" data-tooltip-theme="danger"><i class="fas fa-times me-1"></i> Cancelar</button>
            </div>
        </div>
    </div>
</div>
...
// ===== Select2: filtro "Trabajador" con búsqueda en tiempo real =====
if ($.fn.select2) {
...
const swalDark = { background: 'var(--panel)', color: 'var(--txt)', confirmButtonColor: '#3b82f6', cancelButtonColor: '#6c757d' };

// Notificaciones Flash PHP a JS
document.addEventListener('DOMContentLoaded', function() {
    var tooltipTriggerList = document.documentElement.classList.contains('no-tooltips') ? [] : [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
...
const PUEDE_CREAR_EMPLEADOS    = <?php echo $puede_crear_empleados ? 'true' : 'false'; ?>;
const PUEDE_EDITAR_EMPLEADOS   = <?php echo $puede_editar_empleados ? 'true' : 'false'; ?>;
const PUEDE_ELIMINAR_EMPLEADOS = <?php echo $puede_eliminar_empleados ? 'true' : 'false'; ?>;
let currentIndex = -1; 
let formDirty = false; 
...
function actualizarInfoDesdeFormulario() {
    ...
    const badgeBaja = document.getElementById('badgeBaja');
    if (badgeBaja) {
        const esBaja = !!(fechaBaja && fechaBaja.value && fechaBaja.value.trim() !== '');
        badgeBaja.style.display = esBaja ? 'inline-flex' : 'none';
    }
    ...
    if(alta) {
        ...
        const fechaFin = (fechaBajaInput && fechaBajaInput.value) ? new Date(fechaBajaInput.value) : new Date();
        const fechaAltaDate = new Date(alta);
        let a = fechaFin.getFullYear() - fechaAltaDate.getFullYear();
        let m = fechaFin.getMonth() - fechaAltaDate.getMonth();
        if(m < 0) { a--; m+=12; }
...
function clearForm() {
    ...
    const badgeBaja = document.getElementById('badgeBaja');
    if (badgeBaja) badgeBaja.style.display = 'none';
    ...
    if (imagePreview) {
        ...
        imagePreview.style.padding = '1.25rem';
...
function cargarEmpleadoEnFormulario(emp) {
    ...
    aplicarModoSoloLectura(modoSoloLectura);
}

// Modo solo lectura: permite ver el modal de edición sin poder modificar ningún valor
function aplicarModoSoloLectura(activar) {
    const form = document.getElementById('empleadoForm');
    if (form) {
        const controles = form.querySelectorAll('input:not([type="hidden"]), select, textarea');
        controles.forEach(el => {
            if (activar) {
                if (!el.getAttribute('data-ro-estado')) {
                    el.setAttribute('data-ro-estado', el.disabled ? 'ya-disabled' : 'disabled-por-ro');
                }
                el.disabled = true;
            } else {
                if (el.getAttribute('data-ro-estado') === 'disabled-por-ro') {
                    el.disabled = false;
                }
                el.removeAttribute('data-ro-estado');
            }
        });
    }
    const btnGuardar = document.getElementById('btnGuardarEmpleado');
    if (btnGuardar) btnGuardar.style.display = activar ? 'none' : '';
    const avatarPreview = document.getElementById('avatarPreviewContainer');
    if (avatarPreview) avatarPreview.style.pointerEvents = activar ? 'none' : '';
    const uploadLabel = document.getElementById('imageUpload') ? document.getElementById('imageUpload').closest('label') : null;
    if (uploadLabel) uploadLabel.style.display = activar ? 'none' : '';
    const btnEliminarFoto = document.getElementById('btnEliminarFoto');
    if (btnEliminarFoto) {
        if (activar) {
            btnEliminarFoto.setAttribute('data-ro-original', btnEliminarFoto.style.display || 'none');
            btnEliminarFoto.style.display = 'none';
        } else {
            btnEliminarFoto.style.display = btnEliminarFoto.getAttribute('data-ro-original') || 'none';
        }
    }
...
function editEmpleado(emp) {
    ...
    modoSoloLectura = !PUEDE_EDITAR_EMPLEADOS;
...
document.getElementById('empleadoForm').addEventListener('submit', function(e) {
    ...
    // Validación de Cargo / Técnica antes de enviar
    const cargoInput = document.getElementById('cargo_id');
    if (!cargoInput || !cargoInput.value) {
        Swal.fire(Object.assign({
            icon: 'error',
            title: '<i class="fas fa-briefcase text-danger me-2"></i> Seleccione un Cargo',
            html: `
                <div class="text-start">
                    <p class="mb-2">Debe seleccionar un <strong class="text-info">Cargo / Técnica</strong> para el empleado.</p>
                    <div class="alert alert-warning bg-warning bg-opacity-10 border-warning mt-2 p-2 rounded">
                        <i class="fas fa-info-circle me-1"></i> El cargo es obligatorio y completa automáticamente la escala salarial y la categoría.
                    </div>
                </div>
            `,
            confirmButtonText: '<i class="fas fa-check me-2"></i>Aceptar',
    ...
    .then(d => {
        if(d.success) {
            ...
            Swal.close();
            notificarAccesoDenegado(d.message);
        } else {
            // ============================================
            ...
            html: `
                <div class="text-center">
                    ...
                    <div class="alert alert-danger bg-danger bg-opacity-10 border-danger mt-2 p-2 rounded">
                        <i class="text-light fas fa-exclamation-circle me-1"></i> <span class="text-light">${err.toString()}</span>
                    </div>
...
// =====================================================================
var REPORTE_EMP_EMPRESA = '<?php echo htmlspecialchars($config_empresa['nombre_empresa'] ?? COMPANY_NAME); ?>';
var REPORTE_EMP_USUARIO  = '<?php echo htmlspecialchars($user_nombre_completo ?? "Usuario"); ?>';
var REPORTE_EMP_LOGO     = '<?php echo $logo_base64; ?>';
var REPORTE_EMP_JEFE     = '<?php echo htmlspecialchars($config_empresa['jefe_proyecto'] ?? JEFE_PROYECTO); ?>';
var REPORTE_EMP_ESPECIALISTA = '<?php echo htmlspecialchars($config_empresa['especialista_gestion'] ?? ESPECIALISTA); ?>';
var REPORTE_EMP_ESPECIALISTA_RRHH = '<?php echo htmlspecialchars($config_empresa['especialista_gestionRRHH'] ?? ""); ?>';
...
var REPORTE_EMP_FILAS_POR_PAGINA = 13;
...
var REPORTE_EMP_CSS = `
    ...
    .rep-emp-cabecera-logo { width: 5.625rem !important; padding-right: 0.75rem !important; }
    .rep-emp-cabecera-logo img { width: 2.03cm !important; height: 2.03cm !important; }
    .rep-emp-cabecera-titulo { border-bottom: 0.125rem solid #004B87 !important; padding-bottom: 0.5rem !important; }
    ...
    .rep-emp-alcance { font-size: 8pt !important; color: #444444 !important; margin-top: 0.25rem !important; }
    .rep-emp-cabecera-datos {
        ...
        border-bottom: 0.125rem solid #004B87 !important;
    ...
    .rep-emp-firmas { margin-top: 2.5rem !important; page-break-inside: avoid !important; break-inside: avoid !important; }
    ...
    .rep-emp-firma-label { font-weight: bold !important; margin-bottom: 2.8125rem !important; }
    .rep-emp-firma-linea { border-top: 0.0625rem solid #000000 !important; width: 85% !important; margin: 0 auto !important; padding-top: 0.1875rem !important; }
    .rep-emp-firma-cargo { font-weight: bold !important; font-size: 8pt !important; margin-top: 0.1875rem !important; }
    .rep-emp-firma-subcargo { font-size: 7pt !important; color: #555555 !important; margin-top: 0.0625rem !important; }
    ...
    .btn-print:hover { background: #003366 !important; }
    .btn-print-secondary { background: #666666 !important; }
    .btn-print-secondary:hover { background: #555555 !important; }
    /* Filas con vacaciones >= 20 días (identificación rápida) */
    .rep-emp-excedida td {
...
function reporteEmp_construirContenido(data) {
        ...
        <table class="rep-emp-cabecera" cellspacing="0" cellpadding="0">
            <tr>
                <td class="rep-emp-cabecera-logo">
                    ${REPORTE_EMP_LOGO ? '<img src="' + REPORTE_EMP_LOGO + '" alt="Logo" width="77" height="77" style="width:2.03cm; height:2.03cm;">' : ''}
                </td>
    ...
    for (var p = 0; p < totalPaginas; p++) {
        ...
        filas.forEach(function(row) {
            var excedida = reporteEmp_parseNumero(row[12]) >= 20;
            tbody += '<tr' + (excedida ? ' class="rep-emp-excedida"' : '') + '>';
            row.forEach(function(celda, idx) {
                var clase = REPORTE_EMP_COLS_DERECHA.indexOf(idx) !== -1 ? ' class="text-right"' : '';
                var estilo = excedida ? ' style="background-color:#fde8e8; color:#8b0000;"' : '';
                tbody += '<td' + clase + estilo + '>' + reporteEmp_escHtml(celda) + '</td>';
            });
            tbody += '</tr>';
        });
        ...
        var tfootHtml = '';
        if (ultima) {
            var filaTot = reporteEmp_filaTotalesArray(data);
            var tfoot = '<tr><td colspan="7">' + reporteEmp_escHtml(filaTot[0]) + '</td>';
            for (var ti = 7; ti < filaTot.length; ti++) {
                var tclase = REPORTE_EMP_COLS_DERECHA.indexOf(ti) !== -1 ? ' class="text-right"' : '';
                tfoot += '<td' + tclase + '>' + reporteEmp_escHtml(filaTot[ti]) + '</td>';
            }
            tfoot += '</tr>';
            tfootHtml = '<tfoot>' + tfoot + '</tfoot>';
        }
        html += `
            ...
            <table class="rep-emp-firmas-tabla">
                <tr>
                    <td>
                        ...
                        <p class="rep-emp-firma-cargo">${reporteEmp_escHtml(REPORTE_EMP_ESPECIALISTA_RRHH || REPORTE_EMP_USUARIO)}</p>
                        <p class="rep-emp-firma-subcargo">Especialista de Recursos Humanos</p>
                    </td>
                    <td>
...
function reporteEmp_abrirImpresion(data) {
    ...
    win.document.open();
    win.document.write('<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>LISTADO DE EMPLEADOS</title><style>' + REPORTE_EMP_CSS + '</style></head><body>' + PRINT_TOOLBAR_HTML + contenido + '</body></html>');
    win.document.close();
}
...
function reporteEmp_descargarWord(data) {
    ...
    contenido = contenido.replace(/<div class="print-sheet[ "][^"]*"/g, function(match) {
        if (primeraHoja) { primeraHoja = false; return match; }
        return match.replace('<div class="print-sheet', '<div class="print-sheet rep-word-break');
    });
    var wordCss = REPORTE_EMP_CSS + `
        ...
        .print-sheet-footer { border-top: 0.0625rem solid #000000 !important; padding-top: 0.25rem !important; margin-top: 0.375rem !important; }
        .rep-emp-cabecera-logo { width: 7.5rem !important; }
        .rep-emp-cabecera-logo img { width: 3cm !important; height: 3cm !important; }
    `;
...
function reporteEmp_firmaPdf(label, nombre, cargo) {
    return {
        width: '*',
        stack: [
            { text: label, bold: true, fontSize: 8, alignment: 'center', margin: [0, 0, 0, 45] },
            { table: { widths: ['*'], body: [[{ text: '', height: 1 }]] }, layout: { hLineWidth: function(i) { return (i === 1) ? 0.5 : 0; }, vLineWidth: function() { return 0; } } },
            { text: nombre, bold: true, fontSize: 8, alignment: 'center', margin: [0, 5, 0, 0] },
            { text: cargo, fontSize: 7, color: '#555555', alignment: 'center', margin: [0, 1, 0, 0] }
        ]
    };
}
...
function reporteEmp_calcularTotales(data) {
    var subs = { salario: 7, hora: 8, vacDias: 12, vacImpte: 13, vacPagar: 14, valorDia: 15 };
    var tot = { salario: 0, hora: 0, vacDias: 0, vacImpte: 0, vacPagar: 0, valorDia: 0 };
    if (data && data.body) {
        data.body.forEach(function(row) {
            tot.salario  += reporteEmp_parseNumero(row[subs.salario]);
            tot.hora     += reporteEmp_parseNumero(row[subs.hora]);
            tot.vacDias  += reporteEmp_parseNumero(row[subs.vacDias]);
            tot.vacImpte += reporteEmp_parseNumero(row[subs.vacImpte]);
            tot.vacPagar += reporteEmp_parseNumero(row[subs.vacPagar]);
            tot.valorDia += reporteEmp_parseNumero(row[subs.valorDia]);
        });
    }
    return tot;
}

function reporteEmp_filaTotalesArray(data) {
    var ncols = (data && data.header) ? data.header.length : 19;
    var fila = ['TOTALES (' + ((data && data.body) ? data.body.length : 0) + '):'];
    for (var i = 1; i < ncols; i++) fila.push('');
    var tot = reporteEmp_calcularTotales(data);
    if (7 < ncols) fila[7]   = '$' + tot.salario.toFixed(2);
    if (8 < ncols) fila[8]   = '$' + tot.hora.toFixed(2);
    if (12 < ncols) fila[12] = tot.vacDias.toFixed(2);
    if (13 < ncols) fila[13] = '$' + tot.vacImpte.toFixed(2);
    if (14 < ncols) fila[14] = '$' + tot.vacPagar.toFixed(2);
    if (15 < ncols) fila[15] = '$' + tot.valorDia.toFixed(2);
    return fila;
}

function reporteEmp_exportOptionsConTotales() {
    var opts = reporteEmp_exportOptions();
    opts.customizeData = function(data) {
        data.body.push(reporteEmp_filaTotalesArray(data));
    };
    return opts;
}
...
function reporteEmp_construirTXT(data) {
    ...
    lineas.push('');
    lineas.push('Elaborado por: ' + (REPORTE_EMP_ESPECIALISTA_RRHH || REPORTE_EMP_USUARIO) + ' (Especialista de Recursos Humanos)');
    lineas.push('Revisado por: ' + REPORTE_EMP_ESPECIALISTA + ' (Especialista en Gestión Económica)');
    lineas.push('Aprobado por: ' + REPORTE_EMP_JEFE + ' (Director de Proyecto)');
    ...
    let table = $('#empleadosTable').DataTable({
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            if (footCells.length >= 10) {
                $(footCells[1]).html('$' + fmt(sumarColumna(9)));
                $(footCells[2]).html('$' + fmt(sumarColumna(10)));
                $(footCells[6]).html(fmt(sumarColumna(14)));
                $(footCells[7]).html('$' + fmt(sumarColumna(15)));
                $(footCells[8]).html('$' + fmt(sumarColumna(16)));
                $(footCells[9]).html('$' + fmt(sumarColumna(17)));
                $(footCells[0]).html('TOTALES (' + api.rows({ search: 'applied' }).count() + ' trabajadores):');
            }
        },
        ...
        buttons: [
            ...
            },
            {
                ...
                action: function(e, dt, node, config) {
                    Swal.fire(Object.assign({
                        title: '<i class="fas fa-file-pdf text-danger me-2"></i> Generando PDF del Listado de Empleados',
                        html: '<div class="text-center"><p class="mb-0">Se está generando el PDF, espere unos segundos...</p></div>',
                        allowOutsideClick: false,
                ...
                customize: function(doc) {
                    ...
                    var cabeceraCols = [];
                    if (REPORTE_EMP_LOGO) cabeceraCols.push({ image: REPORTE_EMP_LOGO, width: 57.5, height: 57.5 });
                    ...
                    // Resaltar filas con vacaciones >= 20 días (rojo claro)
                    for (var ci = 0; ci < doc.content.length; ci++) {
                        var tablaPdf = doc.content[ci] ? doc.content[ci].table : null;
                        if (!tablaPdf || !tablaPdf.body) continue;
                        for (var ri = 1; ri < tablaPdf.body.length; ri++) {
                            var filaPdf = tablaPdf.body[ri];
                            var celdaDias = filaPdf[12];
                            var txtDias = (celdaDias && typeof celdaDias === 'object' && celdaDias.text !== undefined) ? celdaDias.text : celdaDias;
                            var nDias = parseFloat(String(txtDias).replace(/[$,\s]/g, '')) || 0;
                            if (nDias >= 20) {
                                for (var cj = 0; cj < filaPdf.length; cj++) {
                                    var celdaPdf = filaPdf[cj];
                                    if (celdaPdf && typeof celdaPdf === 'object') {
                                        celdaPdf.fillColor = '#fde8e8';
                                        celdaPdf.color = '#8b0000';
                                    }
                                }
                            }
                        }
                    ...
                    var dtPdf = $('#empleadosTable').DataTable();
                    var dataPdf = dtPdf.buttons.exportData(reporteEmp_exportOptions());
                    var filaTotPdf = reporteEmp_filaTotalesArray(dataPdf);
                    var pdfTotRow = [{ text: filaTotPdf[0], colSpan: 7, bold: true, alignment: 'left', fillColor: '#dce6f1', color: '#003366' }];
                    for (var pci = 1; pci < 7; pci++) pdfTotRow.push('');
                    for (var pci2 = 7; pci2 < filaTotPdf.length; pci2++) {
                        pdfTotRow.push({
                            text: filaTotPdf[pci2],
                            bold: true,
                            alignment: REPORTE_EMP_COLS_DERECHA.indexOf(pci2) !== -1 ? 'right' : 'left',
                            fillColor: '#dce6f1',
                            color: '#003366'
                        });
                    }
                    if (tablaPdf && tablaPdf.body) tablaPdf.body.push(pdfTotRow);
                    doc.content.push({
                        margin: [0, 28, 0, 0],
                        columns: [
                            reporteEmp_firmaPdf('Elaborado por:', REPORTE_EMP_ESPECIALISTA_RRHH || REPORTE_EMP_USUARIO, 'Especialista de Recursos Humanos'),
                            reporteEmp_firmaPdf('Revisado por:', REPORTE_EMP_ESPECIALISTA, 'Especialista en Gestión Económica'),
                            reporteEmp_firmaPdf('Aprobado por:', REPORTE_EMP_JEFE, 'Director de Proyecto')
                        ]
                    });
                }
            },
            ...
            },
            {
                ...
                customize: function(xlsx) {
                    try {
                        ...
                        if (!sheetDoc || !sheetDoc.getElementsByTagName) return;

                        // Configurar página: horizontal y ajustar a una página
                        var sheetElems = sheetDoc.getElementsByTagName('worksheet');
                        if (sheetElems.length && !sheetDoc.getElementsByTagName('pageSetup').length) {
                            var ps = sheetDoc.createElement('pageSetup');
                            ps.setAttribute('orientation', 'landscape');
                            ps.setAttribute('paperSize', '1');
                            ps.setAttribute('fitToWidth', '1');
                            ps.setAttribute('fitToHeight', '1');
                            sheetElems[0].appendChild(ps);
                        }

                        var rows = sheetDoc.getElementsByTagName('row');

                        // Expediente (A) y CI (B) como texto para no perder ceros a la izquierda ni notación científica
                        var colsTexto = ['A', 'B'];
                        for (var rri = 0; rri < rows.length; rri++) {
                            var rowT = rows[rri];
                            var rN = parseInt(rowT.getAttribute('r'), 10);
                            if (!rN || rN === 1) continue;
                            var cellsT = rowT.getElementsByTagName('c');
                            if (!cellsT.length) continue;
                            var uniformeT = true;
                            for (var cti = 0; cti < cellsT.length; cti++) {
                                if (cellsT[cti].getAttribute('s') !== '2') uniformeT = false;
                            }
                            if (uniformeT) continue;
                            for (var cti2 = 0; cti2 < cellsT.length; cti2++) {
                                var cellT = cellsT[cti2];
                                var refT = cellT.getAttribute('r') || '';
                                var esColTexto = false;
                                for (var ciT = 0; ciT < colsTexto.length; ciT++) {
                                    if (refT.indexOf(colsTexto[ciT]) === 0) { esColTexto = true; break; }
                                }
                                if (!esColTexto) continue;
                                var vT = cellT.getElementsByTagName('v')[0];
                                var isT = cellT.getElementsByTagName('is')[0];
                                var textoVal = '';
                                if (isT) {
                                    var tT = isT.getElementsByTagName('t')[0];
                                    textoVal = tT ? (tT.textContent || (tT.firstChild && tT.firstChild.nodeValue) || '') : '';
                                } else if (vT) {
                                    textoVal = vT.textContent || (vT.firstChild && vT.firstChild.nodeValue) || '';
                                } else {
                                    continue;
                                }
                                cellT.removeAttribute('s');
                                cellT.setAttribute('t', 'inlineStr');
                                while (cellT.firstChild) cellT.removeChild(cellT.firstChild);
                                var isNuevo = sheetDoc.createElement('is');
                                var tNuevo = sheetDoc.createElement('t');
                                tNuevo.appendChild(sheetDoc.createTextNode(textoVal));
                                isNuevo.appendChild(tNuevo);
                                cellT.appendChild(isNuevo);
                            }
                        }

                        // Filas con vacaciones >= 20 días (fondo rojo claro)
                        var redRows = [];
                        for (var ri = 0; ri < rows.length; ri++) {
                            ...
                            for (var ci = 0; ci < cells.length; ci++) {
                                if (cells[ci].getAttribute('s') !== '2') uniforme = false;
                                if ((cells[ci].getAttribute('r') || '').indexOf('M') === 0) mCell = cells[ci];
                            }
                            if (uniforme || !mCell) continue;
                            var vEl = mCell.getElementsByTagName('v')[0];
                            var tEl = mCell.getElementsByTagName('t')[0];
                            var texto = vEl ? (vEl.textContent || (vEl.firstChild && vEl.firstChild.nodeValue) || '') : (tEl ? (tEl.textContent || '') : '');
                            var num = parseFloat(String(texto).replace(/[$,\s]/g, '').trim());
                            if (isNaN(num) || num < 20) continue;
                            redRows.push(row);
                        }

                        // Fila de totales al final (etiqueta fusionada A:G = colspan 7)
                        var dtX = $('#empleadosTable').DataTable();
                        var dataX = dtX.buttons.exportData(reporteEmp_exportOptions());
                        var totX = reporteEmp_calcularTotales(dataX);
                        var sheetDataEl = sheetDoc.getElementsByTagName('sheetData')[0];
                        if (sheetDataEl) {
                            var rowsX = sheetDataEl.getElementsByTagName('row');
                            var maxRX = 0;
                            for (var rxi = 0; rxi < rowsX.length; rxi++) {
                                var rv = parseInt(rowsX[rxi].getAttribute('r'), 10);
                                if (rv > maxRX) maxRX = rv;
                            }
                            var newRX = maxRX + 1;
                            var newRowX = sheetDoc.createElement('row');
                            newRowX.setAttribute('r', String(newRX));
                            var addCellX = function(colLetter, value, isNum) {
                                var cEl = sheetDoc.createElement('c');
                                cEl.setAttribute('r', colLetter + newRX);
                                if (isNum) {
                                    var vEl = sheetDoc.createElement('v');
                                    vEl.appendChild(sheetDoc.createTextNode(String(value)));
                                    cEl.appendChild(vEl);
                                } else {
                                    cEl.setAttribute('t', 'inlineStr');
                                    var isEl = sheetDoc.createElement('is');
                                    var tEl = sheetDoc.createElement('t');
                                    tEl.appendChild(sheetDoc.createTextNode(String(value)));
                                    isEl.appendChild(tEl);
                                    cEl.appendChild(isEl);
                                }
                                newRowX.appendChild(cEl);
                            };
                            addCellX('A', 'TOTALES (' + dataX.body.length + '):', false);
                            addCellX('H', Math.round(totX.salario * 100) / 100, true);
                            addCellX('I', Math.round(totX.hora * 100) / 100, true);
                            addCellX('M', Math.round(totX.vacDias * 100) / 100, true);
                            addCellX('N', Math.round(totX.vacImpte * 100) / 100, true);
                            addCellX('O', Math.round(totX.vacPagar * 100) / 100, true);
                            addCellX('P', Math.round(totX.valorDia * 100) / 100, true);
                            var lastRowX = rowsX.length ? rowsX[rowsX.length - 1] : null;
                            if (lastRowX) sheetDataEl.insertBefore(newRowX, lastRowX.nextSibling);
                            else sheetDataEl.appendChild(newRowX);
                            var mergeCellsEl = sheetDoc.getElementsByTagName('mergeCells')[0];
                            if (!mergeCellsEl) {
                                mergeCellsEl = sheetDoc.createElement('mergeCells');
                                mergeCellsEl.setAttribute('count', '1');
                                var wsEl = sheetDoc.getElementsByTagName('worksheet')[0];
                                if (wsEl) wsEl.insertBefore(mergeCellsEl, sheetDataEl.nextSibling);
                            } else {
                                var cntMerge = parseInt(mergeCellsEl.getAttribute('count') || '0', 10);
                                mergeCellsEl.setAttribute('count', String((isNaN(cntMerge) ? 0 : cntMerge) + 1));
                            }
                            var mergeCellEl = sheetDoc.createElement('mergeCell');
                            mergeCellEl.setAttribute('ref', 'A' + newRX + ':G' + newRX);
                            mergeCellsEl.appendChild(mergeCellEl);
                        }
                        if (!redRows.length || !stylesDoc || !stylesDoc.getElementsByTagName) return;

                        var used = { '0': true };
                        for (var i = 0; i < redRows.length; i++) {
                            var cs = redRows[i].getElementsByTagName('c');
                            for (var j = 0; j < cs.length; j++) {
                                var ss = cs[j].getAttribute('s');
                                if (ss !== null && ss !== undefined) used[ss] = true;
                            }
                        }
                        var cellXfs = stylesDoc.getElementsByTagName('cellXfs')[0];
                        if (!cellXfs) return;
                        var xfNodes = cellXfs.getElementsByTagName('xf');
                        var base = parseInt(cellXfs.getAttribute('count') || '0', 10) || 0;
                        var map = {};
                        var nextIdx = base;
                        for (var k in used) {
                            var src = xfNodes[parseInt(k, 10)];
                            if (!src) { map[k] = null; continue; }
                            var clon = src.cloneNode(true);
                            clon.setAttribute('fillId', '3');
                            clon.setAttribute('applyFill', '1');
                            cellXfs.appendChild(clon);
                            map[k] = nextIdx;
                            nextIdx++;
                        }
                        cellXfs.setAttribute('count', String(nextIdx));
                        for (var i2 = 0; i2 < redRows.length; i2++) {
                            var cs2 = redRows[i2].getElementsByTagName('c');
                            for (var j2 = 0; j2 < cs2.length; j2++) {
                                var old = cs2[j2].getAttribute('s');
                                if (old === null || old === undefined) old = '0';
                                if (map[old] !== null && map[old] !== undefined) {
                                    cs2[j2].setAttribute('s', String(map[old]));
                                }
                            }
                        }
                    } catch (err) {}
            ...
            },
            {
                ...
                exportOptions: reporteEmp_exportOptionsConTotales()
            },
            {
            ...
            },
{
    ...
    // Clic en fila para editar (roles sin permiso lo ven en modo solo lectura)
    ...
    $('.dt-search').html(`
        <div class="input-group input-group-sm" style="width:20rem;">
            ...
            <button class="btn btn-sm btn-outline-secondary" type="button" id="clearSearchBtn" style="background: var(--panel);"><i class="fas fa-times"></i></button>
        </div>
    `);
...
// Filtro por Días de Vacaciones (rangos)
$('#filtroVacacionesExcedidas').on('change', function() {
    var table = $('#empleadosTable').DataTable();
    $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
        return fn.name !== 'filtroVacacionesExcedidas';
    });
    var valor = this.value;
    if (valor === 'mayor20') {
        $.fn.dataTable.ext.search.push(function filtroVacacionesExcedidas(settings, data, dataIndex) {
            var vacDias = parseFloat((data[14] || '').replace(/,/g, '')) || 0;
            return vacDias > 20;
        });
    } else if (valor === 'menor1') {
        $.fn.dataTable.ext.search.push(function filtroVacacionesExcedidas(settings, data, dataIndex) {
            var vacDias = parseFloat((data[14] || '').replace(/,/g, '')) || 0;
            return vacDias > 0 && vacDias < 1;
        });
    } else if (valor !== '') {
        var partesRango = valor.split('-');
        var rangoMin = parseFloat(partesRango[0]);
        var rangoMax = partesRango[1] !== undefined ? parseFloat(partesRango[1]) : rangoMin;
        $.fn.dataTable.ext.search.push(function filtroVacacionesExcedidas(settings, data, dataIndex) {
            var vacDias = parseFloat((data[14] || '').replace(/,/g, '')) || 0;
            return vacDias >= rangoMin && vacDias <= rangoMax;
        });
    }
    table.draw();
});
...
// ==================== EXPORTAR CUMPLEAÑOS (MODAL DE FORMATOS) ====================
let tipoCumpleExport = 'completo'; // 'completo' | '30'

function abrirModalExportarCumpleanos(tipo, infoTexto) {
    tipoCumpleExport = tipo;
    const info = document.getElementById('exportarCumpleInfo');
    if (info) info.innerHTML = infoTexto;
    const modalEl = document.getElementById('exportarCumpleanosModal');
    if (modalEl) {
...
$('#menuExportCumpleCompleto').on('click', function(e) {
    e.preventDefault();
    abrirModalExportarCumpleanos('completo',
        '<i class="fas fa-birthday-cake me-2" style="color: #fbbf24;"></i> Listado Completo de Cumpleaños de todos los trabajadores activos.<br>Seleccione el formato de exportación:'
    );
});

$('#menuExportCumple30').on('click', function(e) {
    e.preventDefault();
    abrirModalExportarCumpleanos('30',
        '<i class="fas fa-calendar-day me-2" style="color: #4ade80;"></i> Cumpleaños que ocurrirán en los próximos 30 días.<br>Seleccione el formato de exportación:'
    );
});

function exportarCumpleanos(formato) {
    const modalEl = document.getElementById('exportarCumpleanosModal');
    const modal = modalEl ? bootstrap.Modal.getInstance(modalEl) : null;
    if (modal) modal.hide();

    Swal.fire({
        title: '<i class="fas fa-spinner fa-pulse me-2" style="color: #fbbf24;"></i> Generando archivo',
        text: 'Por favor espere, se está generando el listado de cumpleaños...',
        allowOutsideClick: false,
        didOpen: () => Swal.showLoading(),
        background: '#1a1a2e',
        color: '#ffffff'
    ...
    formData.append('tipo', tipoCumpleExport);

    fetch('exportar_cumpleanos.php', {
        method: 'POST',
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        body: formData
    })
    ...
    .then(data => {
        if (data.success) {
            const extensiones = { pdf: 'pdf', word: 'doc', excel: 'xlsx', csv: 'csv', txt: 'txt', dbf: 'dbf' };
            const extension = extensiones[formato] || formato;
            const nombreArchivo = data.archivo + '.' + extension;
            fetch(data.descarga)
                .then(res => res.blob())
                .then(blob => {
    ...
    .catch(error => {
        ...
        Swal.fire({
            icon: 'error',
            title: '<i class="fas fa-exclamation-triangle me-2"></i> Error de conexión',
...
// ==================== IMPRIMIR CUMPLEAÑOS (REPORTE HTML) ====================
var REPORTE_CUMPLE_FILAS_POR_PAGINA = 15;

function reporteCumple_escHtml(t) {
    if (t === null || t === undefined) return '';
    return $('<div>').text(String(t)).html();
}

function reporteCumple_datos(tipo) {
    var lista = (typeof cumpleaneros !== 'undefined' && Array.isArray(cumpleaneros)) ? cumpleaneros : [];
    if (tipo === '30') {
        lista = lista.filter(function(c) { return Number(c.dias) <= 30; });
    }
    return lista;
}

function reporteCumple_titulo(tipo) {
    return tipo === '30' ? 'CUMPLEAÑOS PRÓXIMOS 30 DÍAS' : 'LISTADO COMPLETO DE CUMPLEAÑOS';
}

function reporteCumple_construirContenido(datos, tipo) {
    var fechaHora = reporteEmp_fechaHora12h();
    var total = datos.length;
    var titulo = reporteCumple_titulo(tipo);
    var subtitulo = tipo === '30'
        ? 'Cumpleaños de los trabajadores activos que ocurrirán en los próximos 30 días.'
        : 'Cumpleaños de todos los trabajadores activos (no incluye bajas), calculados desde el CI.';
    ...
    html += `
        <table class="rep-emp-cabecera" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td class="rep-emp-cabecera-titulo">
                    <div class="rep-emp-empresa">${reporteCumple_escHtml(REPORTE_EMP_EMPRESA)}</div>
                    <div class="rep-emp-titulo">${reporteCumple_escHtml(titulo)}</div>
                    <div class="rep-emp-alcance">${reporteCumple_escHtml(subtitulo)}</div>
                </td>
                <td class="rep-emp-cabecera-datos">
                    <strong>Emisión:</strong> ${fechaHora}<br>
                    <strong>Generado por:</strong> ${reporteCumple_escHtml(REPORTE_EMP_USUARIO)}<br>
                    <strong>Total Cumpleañeros:</strong> <span class="rep-emp-total">${total}</span>
                </td>
            </tr>
        </table>`;

    var totalPaginas = Math.max(1, Math.ceil(datos.length / REPORTE_CUMPLE_FILAS_POR_PAGINA));
    for (var p = 0; p < totalPaginas; p++) {
        var filas = datos.slice(p * REPORTE_CUMPLE_FILAS_POR_PAGINA, (p + 1) * REPORTE_CUMPLE_FILAS_POR_PAGINA);
        var tbody = '';
        filas.forEach(function(c, i) {
            var num = p * REPORTE_CUMPLE_FILAS_POR_PAGINA + i + 1;
            tbody += '<tr>'
                + '<td class="text-right">' + num + '</td>'
                + '<td>' + reporteCumple_escHtml(c.nombre) + '</td>'
                + '<td>' + reporteCumple_escHtml(c.fecha) + '</td>'
                + '<td class="text-right">' + reporteCumple_escHtml(c.edad) + '</td>'
                + '<td class="text-right">' + reporteCumple_escHtml(c.dias) + '</td>'
                + '</tr>';
        });
        var ultima = p === totalPaginas - 1;
        html += `
            <div class="print-sheet ${ultima ? 'print-sheet-last' : ''}">
                ...
                <div class="print-sheet-footer">
                    <div>${reporteCumple_escHtml(titulo)} - ${reporteCumple_escHtml(REPORTE_EMP_EMPRESA)}</div>
                    <div class="print-sheet-footer-center">Página ${p + 1} de ${totalPaginas}</div>
                    <div class="print-sheet-footer-right">Impresión: ${fechaHora}</div>
                </div>
            </div>`;
            ...
            <table class="rep-emp-firmas-tabla">
                <tr>
                    <td>
                        <p class="rep-emp-firma-label">Elaborado por:</p>
                        <p class="rep-emp-firma-linea"></p>
                        <p class="rep-emp-firma-cargo">${reporteCumple_escHtml(REPORTE_EMP_ESPECIALISTA_RRHH || REPORTE_EMP_USUARIO)}</p>
                        <p class="rep-emp-firma-subcargo">Especialista de Recursos Humanos</p>
                    </td>
                    <td>
                        <p class="rep-emp-firma-label">Revisado por:</p>
                        <p class="rep-emp-firma-linea"></p>
                        <p class="rep-emp-firma-cargo">${reporteCumple_escHtml(REPORTE_EMP_ESPECIALISTA)}</p>
                        <p class="rep-emp-firma-subcargo">Especialista en Gestión Económica</p>
                    </td>
                    <td>
                        <p class="rep-emp-firma-label">Aprobado por:</p>
                        <p class="rep-emp-firma-linea"></p>
                        <p class="rep-emp-firma-cargo">${reporteCumple_escHtml(REPORTE_EMP_JEFE)}</p>
                        <p class="rep-emp-firma-subcargo">Director de Proyecto</p>
                    </td>
                </tr>
            </table>
...
function reporteCumple_abrirImpresion(tipo) {
    var datos = reporteCumple_datos(tipo);
    if (!datos.length) {
        Swal.fire(Object.assign({
            icon: 'warning',
            title: '<i class="fas fa-birthday-cake me-2" style="color:#fbbf24;"></i> Sin datos',
            text: tipo === '30' ? 'No hay cumpleaños en los próximos 30 días.' : 'No hay trabajadores con cumpleaños para imprimir.',
            confirmButtonText: '<i class="fas fa-check me-2"></i>Entendido',
            confirmButtonColor: '#3b82f6',
            background: '#1a1a2e',
            color: '#ffffff'
    ...
    var contenido = reporteCumple_construirContenido(datos, tipo);
    var titulo = reporteCumple_titulo(tipo);
    var win = window.open('', '_blank');
    if (!win) {
        Swal.fire(Object.assign({
    ...
    win.document.open();
    win.document.write('<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>' + titulo + '</title><style>' + REPORTE_EMP_CSS + '</style></head><body>' + PRINT_TOOLBAR_HTML + contenido + '</body></html>');
    win.document.close();
}

function imprimirCumpleanos() {
    const modalEl = document.getElementById('exportarCumpleanosModal');
    const modal = modalEl ? bootstrap.Modal.getInstance(modalEl) : null;
    if (modal) modal.hide();
    reporteCumple_abrirImpresion(tipoCumpleExport);
}

// ==================== BOTÓN EXPORTAR/IMPRIMIR DEL CARD CUMPLEAÑOS ====================
$('#cardCumpleExportCompleto').on('click', function(e) {
    e.preventDefault();
    abrirModalExportarCumpleanos('completo',
        '<i class="fas fa-birthday-cake me-2" style="color: #fbbf24;"></i> Listado Completo de Cumpleaños de todos los trabajadores activos.<br>Seleccione el formato de exportación:'
    );
...
$('#cardCumpleExport30').on('click', function(e) {
    e.preventDefault();
    abrirModalExportarCumpleanos('30',
        '<i class="fas fa-calendar-day me-2" style="color: #4ade80;"></i> Cumpleaños que ocurrirán en los próximos 30 días.<br>Seleccione el formato de exportación:'
    );
...
$('#cardCumplePrintCompleto').on('click', function(e) {
    e.preventDefault();
    reporteCumple_abrirImpresion('completo');
});

$('#cardCumplePrint30').on('click', function(e) {
    e.preventDefault();
    reporteCumple_abrirImpresion('30');
});
// ==================== FIN BOTÓN CARD CUMPLEAÑOS ====================
// ==================== FIN EXPORTAR CUMPLEAÑOS ====================
...
function mostrarModalFoto(url, nombre, empleadoId, tieneFotoReal, esLogoPorDefecto = false) {
    const logoHtml = esLogoPorDefecto ? 
        `<div style="width:15.625rem; height:15.625rem; background: linear-gradient(135deg, #1a1a2e, #2d2d44); border-radius: 1rem; border: 0.1875rem solid #60a5fa; margin:0.625rem auto; display: flex; align-items: center; justify-content: center; flex-direction: column;">
            <img src="${url}" style="width:7.5rem; height:auto; object-fit: contain; opacity: 0.9;" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\'%3E%3Ccircle cx=\'50\' cy=\'50\' r=\'45\' fill=\'%233b82f6\'/%3E%3Ctext x=\'50\' y=\'65\' text-anchor=\'middle\' fill=\'white\' font-size=\'40\' font-family=\'Arial\'%3E👤%3C/text%3E%3C/svg%3E'">
            <span style="margin-top:0.625rem; font-size:0.75rem; color: #60a5fa;"><i class="fas fa-info-circle me-1"></i>Sin foto registrada</span>
        </div>` :
        (tieneFotoReal ? 
            `<img src="${url}" style="width:15.625rem; height:15.625rem; object-fit: cover; border-radius: 1rem; border: 0.1875rem solid #60a5fa; margin:0.625rem auto; display: block;">` :
            `<div style="width:15.625rem; height:15.625rem; background: rgba(255,255,255,0.05); border-radius: 1rem; border: 0.1875rem solid #60a5fa; margin:0.625rem auto; display: flex; align-items: center; justify-content: center; flex-direction: column;">
                <i class="fas fa-user-circle" style="font-size:5rem; color: rgba(255,255,255,0.3);"></i>
                <span style="margin-top:0.625rem; font-size:0.8125rem; color: rgba(255,255,255,0.5);">Sin foto</span>
            </div>`);
    
    Swal.fire({
        title: `<div style="display: flex; align-items: center; justify-content: center; gap:0.625rem;">
        ...
        html: `
            <div style="text-align: center;">
                ${logoHtml}
                <div class="mt-4 d-flex gap-2 justify-content-center" style="display: flex; gap:0.625rem; justify-content: center; margin-top:0.9375rem; flex-wrap: wrap;">
                    <button type="button" class="btn-win btn-win-primary btn-sm" id="btnVerSolapinModal" style="cursor: pointer; padding:0.375rem 1rem;">
                        <i class="fas fa-id-card me-2"></i> Ver Solapín
                    </button>
                    <button type="button" class="btn-win btn-win-primary btn-sm" id="btnExportarSolapinModal" style="cursor: pointer; padding:0.375rem 1rem;">
                        <i class="fas fa-download me-2"></i> Exportar Solapín
                    </button>
                    <button type="button" class="btn-win btn-win-primary btn-sm" id="btnExportarWordSolapinModal" style="cursor: pointer; padding:0.375rem 1rem;">
                        <i class="fas fa-file-word me-2"></i> Exportar a Word
                    </button>
                    <button type="button" class="btn-win btn-win-primary btn-sm" id="btnImprimirSolapinModal" style="cursor: pointer; padding:0.375rem 1rem;">
...
// Botón Exportar Solapín con tamaño calibrado a 7cm x 20.5cm (16.5625rem x 48.4375rem)
...
function actualizarSolapin() {
    ...
    if (solapinCargoSpan) {
        solapinCargoSpan.textContent = cargoTexto ? ` ${cargoTexto}` : ' --';
    }
}
// Actualizar la foto del solapín
function actualizarSolapinFoto() {
    ...
    if (solapinFotoImg) {
        ...
        solapinFotoImg.style.width = '3.75rem';
        solapinFotoImg.style.height = '3.75rem';
        solapinFotoImg.style.border = '0.125rem solid #60a5fa';
    }
}
...
window.exportarSolapinAWord = function(empleadoId, empleadoNombre, fotoUrl) {
    ...
    <body>
        <div class="Section1">
            <h2 style="font-size:14pt; font-weight:bold; color:#004B87; margin-bottom:0.3125rem;">Solapín Oficial de Identificación</h2>
            <p style="font-size:10pt; color:#333333; margin-bottom:0.9375rem;">Empleado: <strong>${escapeHtml(empleadoNombre)}</strong></p>
    ...
    Swal.fire({
        ...
        html: `
            <div class="text-center px-2">
                ...
                <div class="p-3 my-3 rounded text-start" style="background: rgba(239, 68, 68, 0.08); border: 0.0625rem solid rgba(239, 68, 68, 0.25); border-radius: 0.75rem;">
                    <div class="d-flex align-items-start gap-3">
                        <div style="flex-shrink: 0; margin-top:0.1875rem;">
                        ...
                        <div>
                            ...
                            <span style="font-size:0.78rem; color: rgba(255, 255, 255, 0.7); line-height:1.45; display: block;">
                                ${mainText}.<br><br>
                                ${pathText ? `Ruta requerida en el servidor:<br><code style="background: rgba(0,0,0,0.3); padding:0.125rem 0.375rem; border-radius: 0.25rem; color: #ff8888; font-family: monospace; display: inline-block; margin-top:0.25rem; word-break: break-all;">${pathText}</code>` : ''}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                Swal.fire({
                    ...
                    html: `
                        <div class="text-center">
                            ...
                            <div id="solapinScrollContainer" style="display: inline-block; width:20rem; height:35rem; background: rgba(0, 0, 0, 0.45); border: 0.0625rem solid rgba(255, 255, 255, 0.08); border-radius: 0.75rem; box-shadow: 0 0.5rem 2rem rgba(0,0,0,0.6); overflow: hidden; cursor: grab; user-select: none; position: relative;">
                                <div style="display: inline-block; min-width:100%; min-height:100%; text-align: center; white-space: nowrap; vertical-align: middle;">
                                    ...
                                    <!-- 1. Altura de la imagen a 62.5rem (200% de la altura base de 31.25rem) -->
									<img id="previewSolapinImg" src="${urlSolapin}" draggable="false" style="display: inline-block; vertical-align: middle; width:auto; height:62.5rem; border-radius: 0.375rem; pointer-events: none; transition: height 0.12s ease-out;">
                                </div>
                            </div>

                            <!-- Porcentaje del zoom -->
							<!-- 2. Texto de escala inicial por defecto a 200% -->
							<div style="margin-top:0.9375rem; font-size:0.75rem; color: rgba(255, 255, 255, 0.65);">
...
function actualizarFilaTablaHTML(emp) {
    ...
    if (row.length) {
        ...
        let estadoBadge = emp.activo == 1 && (!emp.fecha_baja || emp.fecha_baja > hoyStr) ? 
            '<span class="badge" style="background: rgba(16, 185, 129, 0.2); color: 4;"><i class="fas fa-check-circle me-1"></i>Activo</span>' :
...
window.vincularCargoPlantilla = function(cargoId) {
    // Si se selecciona "-- Seleccionar Cargo --", limpiar escala y categoría
    if (!cargoId) {
        const selectEscala = document.getElementById('escala_id');
        const selectCategoria = document.getElementById('categoria_id');
        if (selectEscala) selectEscala.value = '';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            ventanaImpresion.document.write(`
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Impresión Masiva de Solapines - SisGesNom</title>
                    <style>
                        @page {
                            size: letter portrait;
                            margin:0;
...
$('#menuExportTodosSolapines').on('click', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Swal.fire({
                title: '<strong><i class="fas fa-spinner fa-pulse text-info me-2"></i> Verificando personal activo</strong>',
                html: '<div class="text-center"><p class="mb-0">Comprobando trabajadores activos en la base de datos...</p></div>',
                allowOutsideClick: false,
                didOpen: () => Swal.showLoading(),
                background: '#1a1a2e',
                color: '#ffffff'
            ...
            // Verificar primero si existen trabajadores activos
            $.ajax({
                url: 'solapines.php',
                type: 'GET',
                data: { exportar_todos: 1, verificar: 1 },
                dataType: 'json'
            }).done(function(res) {
                if (res && res.success === false) {
                    Swal.fire({
                        icon: 'warning',
                        title: '<i class="fas fa-users-slash text-warning me-2"></i> Sin trabajadores activos',
                ...
                Swal.fire({
                    icon: 'error',
                    title: '<i class="fas fa-times-circle text-danger me-2"></i> Error de conexión',
                    text: 'No se pudo comprobar el personal activo. Intente nuevamente.',
                    confirmButtonText: '<i class="fas fa-check me-2"></i> Entendido',
                    confirmButtonColor: '#dc3545',
                    background: '#1a1a2e',
                    color: '#ffffff'
...
function renderCumpleanero(index) {
    ...
    var fotoHtml = fotoSrc
        ? `<img src="${fotoSrc}" alt="${item.nombre}" class="rounded-circle" style="width:9.375rem; height:9.375rem; object-fit: cover; border: 0.1875rem solid rgba(255,255,255,0.15);" onerror="this.onerror=null; this.outerHTML='<div class=\\'rounded-circle d-flex align-items-center justify-content-center\\' style=\\'width:9.375rem;height:9.375rem;border:0.1875rem solid rgba(255,255,255,0.15);background:linear-gradient(135deg,rgba(251,191,36,0.25),rgba(255,255,255,0.05));\\'><i class=\\'fas fa-birthday-cake\\' style=\\'font-size:4.375rem;color:#fbbf24;\\'></i></div>';">`
        : `<div class="rounded-circle d-flex align-items-center justify-content-center" style="width:9.375rem;height:9.375rem;border:0.1875rem solid rgba(255,255,255,0.15);background:linear-gradient(135deg,rgba(251,191,36,0.25),rgba(255,255,255,0.05));"><i class="fas fa-birthday-cake" style="font-size:4.375rem;color:#fbbf24;"></i></div>`;
    ...
    var html = `
        <div class="d-flex flex-column align-items-center">
            ...
            <div style="width:100%; min-height:2.7em; display:flex; align-items:flex-start; justify-content:center;">
                <h5 class="mb-0" style="cursor: pointer; color: #60a5fa; line-height:1.35; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;" onclick="abrirEmpleado(${item.id})">${item.nombre}</h5>
            </div>
    ...
    document.getElementById('lastCumple')?.addEventListener('click', function() {
        if (cumpleIndex !== cumpleaneros.length - 1) {
            cumpleIndex = cumpleaneros.length - 1;
            renderCumpleanero(cumpleIndex);
            renderLista();
        }
    });
...
function renderLista() {
    ...
    top10.forEach(function(emp, idx) {
        var fotoMini = emp.foto && emp.foto.trim() !== ''
            ? `<img src="${emp.foto.trim()}" style="width:2.5rem; height:2.5rem; object-fit: cover; border-radius: 50%; border: 0.0625rem solid rgba(255,255,255,0.1);" onerror="this.onerror=null; this.outerHTML='<i class=\\'fas fa-birthday-cake\\' style=\\'font-size: 1.375rem; color: #fbbf24; width: 2.5rem; text-align: center;\\'></i>';">`
            : `<i class="fas fa-birthday-cake" style="font-size:1.375rem; color: #fbbf24; width:2.5rem; text-align: center;"></i>`;
        ...
        html += `
            <div class="d-flex align-items-center gap-2 p-1 mb-1 rounded ${activeClass}" style="cursor: pointer; background: ${idx === cumpleIndex ? 'rgba(96,165,250,0.15)' : 'transparent'}; transition: background 0.2s;" onclick="irACumpleanero(${idx})">
                ...
                <div style="flex:1; min-width:0;">
                    ...
                    <div style="font-size:0.85rem; color: rgba(255,255,255,0.5); display: flex; gap:0.5rem;">
...
$(document).ready(function() {
    setTimeout(function() {
        if (typeof clearFormAndStartNew === 'function') {
            clearFormAndStartNew();
        }
        var modalNuevo = document.getElementById('empleadoModal');
        if (modalNuevo) {
            var bModal = new bootstrap.Modal(modalNuevo);
            bModal.show();
            modalNuevo.addEventListener('shown.bs.modal', function onShown() {
                var ciInput = document.getElementById('ci');
                if (ciInput) {
                    ciInput.focus();
                    ciInput.select();
                }
                modalNuevo.removeEventListener('shown.bs.modal', onShown);
            });
        }
    }, 500);
});
...
<div class="scroll-quick-btns">
    <button type="button" class="scroll-quick-btn" id="btnScrollTop" title="Ir al principio">
        <i class="fas fa-arrow-up"></i>
    </button>
    <button type="button" class="scroll-quick-btn" id="btnScrollBottom" title="Ir al final">
