<?php
// config/database.php - Configuración central del sistema y conexión PDO.
// Las credenciales de conexión (host, nombre BD, usuario y contraseña) se
// leen desde config.php, que es el único lugar donde se definen.

// Si no existe config.php, mostrar el aviso de configuración no encontrada
// y redirigir al instalador (sin lanzar error fatal).
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left;">
                    <p style="color: #f87171; margin-bottom:0.625rem;">
                        <i class="fas fa-exclamation-triangle"></i> 
                        <strong>Error de conexión:</strong>
                    </p>
                    <div style="background: rgba(0,0,0,0.3); padding:0.75rem; border-radius: 0.5rem; margin:0.9375rem 0;">
                        <i class="fas fa-code me-2"></i>
                        <span style="font-family: monospace; font-size:0.8125rem;"><?php echo addslashes($error_message); ?></span>
                    </div>
                    <hr style="border-color: rgba(255,255,255,0.1); margin:0.9375rem 0;">
            ...
            showDenyButton: true,
            denyButtonText: '<i class="fas fa-database me-2"></i> Instalar BD',
            ...
            if (result.isConfirmed) {
                ...
                var w = window.open('<?php echo BASE_URL; ?>/../InstalarBD/InstalarBD.php', '_blank');
                if (!w) { window.location.href = '<?php echo BASE_URL; ?>/../InstalarBD/InstalarBD.php'; }
            }
        });
        </script>
    </body>
...
// ============================================
// CONEXIÓN PDO (usando las constantes de config.php)
// ============================================
$pdo = null;
try {
    $pdo = new PDO(
    ...
    if (strpos($error_msg, 'denegó expresamente') !== false) {
        ...
        $error_msg = '🔒 Credenciales de acceso incorrectas. Verifique el usuario y la contraseña.';
...
/**
 * Obtiene un valor de configuración en crudo (sin conversión de tipo)
 */
...
// Constantes del sistema (con fallback a valores por defecto)
if (!defined('SITE_NAME'))     define('SITE_NAME', $configs['nombre_empresa'] ?? 'SisGesNom');
if (!defined('SITE_VERSION'))  define('SITE_VERSION', 'v2.0.1');
if (!defined('COMPANY_NAME'))  define('COMPANY_NAME', $configs['nombre_empresa'] ?? 'SisGesNom');
if (!defined('COMPANY_ADDRESS')) define('COMPANY_ADDRESS', $configs['direccion_empresa'] ?? 'S/D');
if (!defined('NIT'))           define('NIT', $configs['nit_empresa'] ?? '319-1-02264');
...
if (!defined('EMAIL_EMPRESA')) define('EMAIL_EMPRESA', $configs['email_empresa'] ?? 'S/R');
if (!defined('SLOGAN'))        define('SLOGAN', $configs['slogan'] ?? 'Eslogan Corporativo e Identificativo');

// Constantes de configuración laboral
if (!defined('HORAS_MENSUALES'))       define('HORAS_MENSUALES', $configs['horas_mensuales'] ?? 192);
if (!defined('DIAS_MENSUALES'))        define('DIAS_MENSUALES', $configs['dias_mensuales'] ?? 24);
if (!defined('HORAS_JORNADA_DIARIA'))  define('HORAS_JORNADA_DIARIA', $configs['horas_jornada_diaria'] ?? 8);
if (!defined('SALARIO_MINIMO'))        define('SALARIO_MINIMO', $configs['salario_minimo'] ?? 2100);
if (!defined('TASA_CONTRIBUCION_ESPECIAL')) define('TASA_CONTRIBUCION_ESPECIAL', $configs['tasa_contribucion_especial'] ?? 5);

// ============================================
// FUNCIONES DEL SISTEMA (USAN LAS CONSTANTES)
// ============================================

function getHorasMensuales($pdo = null) {
    if ($pdo instanceof PDO) {
        return (int) getConfig($pdo, 'horas_mensuales', HORAS_MENSUALES);
    }
