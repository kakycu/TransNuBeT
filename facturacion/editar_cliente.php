<?php
// editar_cliente.php - Windows 11 Dark Mode - Full Version Final 2026
...
if (empty($id_cliente)) {
    ...
    <body>
        ...
        Swal.fire({
            title: "Error",
            text: "No se ha especificado un ID de cliente válido.",
            icon: "error",
            background: "#1f1f1f",
            color: "#fff",
...
if (!is_numeric($id_cliente)) {
    ...
    <body>
        ...
        Swal.fire({
            title: "Error",
            text: "El ID del cliente debe ser un número válido.",
            icon: "error",
            background: "#1f1f1f",
            color: "#fff",
            confirmButtonText: "<i class=\'fas fa-check\'></i> Aceptar",
...
try {
    ...
    if (!$esAdmin && !$esEditor && !$esSuper) {
        $_SESSION['swal_no_privilegios'] = [
            ...
            'mensaje' => 'Solo usuarios con rol de <strong>Administrador, Editor o Supervisor</strong> pueden editar Clientes.',
...
$sql_cliente = "SELECT * FROM clasif_clientes WHERE id = :id";
...
if (!$cliente) {
    ...
    <body>
        ...
        Swal.fire({
            title: "Error",
            text: "El cliente con ID ' . $id_cliente . ' no existe en la base de datos.",
            icon: "error",
            background: "#1f1f1f",
            color: "#fff",
            confirmButtonText: "<i class=\'fas fa-check\'></i> Aceptar",
    ...
    $total_facturas = 0; $total_clientes = 0; $total_categorias = 0; $total_servicios = 0; $total_usuarios = 0; $total_historico = 0;

    try {
        $stmt = $db->query("SELECT COUNT(*) as total FROM tbl_fact"); $total_facturas = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        $stmt = $db->query("SELECT COUNT(*) as total FROM clasif_clientes"); $total_clientes = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        $stmt = $db->query("SELECT COUNT(*) as total FROM clasif_cat_de_serv"); $total_categorias = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        $stmt = $db->query("SELECT COUNT(*) as total FROM clasif_serv"); $total_servicios = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        $stmt = $db->query("SELECT COUNT(*) as total FROM clasif_usuarios"); $total_usuarios = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
        $stmt = $db->query("SELECT COUNT(*) as total FROM historico_operaciones"); $total_historico = $stmt->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;
    } catch (Exception $e) {}
...
if ($fecha_final_total < $hoy) {
    ...
    // Estado contractual (basado en fecha)
    $estado_contrato = 'VENCIDO';
    $estado_contrato_color = 'estado-vencido';
} else {
    // Aún no vence
    $es_vencido = false;
    ...
    $estado_contrato_color = 'estado-activo';
}

// Estado del cliente en BD (separado del estado contractual)
$estado_cliente_bd = ($cliente['activo'] == 1) ? 'ACTIVO' : 'INACTIVO';
$estado_cliente_bd_color = ($cliente['activo'] == 1) ? 'estado-activo' : 'estado-inactivo';

// Fechas para el resumen
$fecha_inicio_php = $cliente['fechaRegistro'];
$fecha_fin_php = $cliente['fechafinalcontrato'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editar'])) {
    try {
        ...
        // 2. Validar Vigencia (Entero > 0)
        if (!is_numeric($_POST['vigenciapor']) || (int)$_POST['vigenciapor'] <= 0 || strpos($_POST['vigenciapor'], '.') !== false) {
            throw new Exception("La <strong>Vigencia (Años)</strong> debe ser un número entero mayor a 0.");
        }

        // 3. Validar Código REEUP (Formato específico)
        ...
        // 4. Validar NIT (Solo números, longitud específica si se proporciona)
        if (!empty($_POST['NIT'])) {
        ...
        // 5. Validar Teléfono (Formato: + y 8 o más números)
        ...
        // 5.1. Validar Carné de Identidad (NoCIResp) - VALIDACIÓN DIRECTA CON $_POST
        if (!empty($_POST['NoCIResp'])) {
            $noci = preg_replace('/\s+/', '', $_POST['NoCIResp']);
            
            if (!ctype_digit($noci)) {
                throw new Exception("El <strong>Carné de Identidad (CI)</strong> debe contener solo números.");
            }
            
            if (strlen($noci) != 11) {
                throw new Exception("El <strong>Carné de Identidad (CI)</strong> debe tener exactamente 11 dígitos.");
            }
            
            // Guardamos el valor limpio en $_POST para usarlo después
            $_POST['NoCIResp'] = $noci;
        }
        // 6. Validar Cuenta Deudor (Opcional, pero si hay dato: solo números y longitud)
        ...
        // 7. Validar Sucursal (Opcional, pero si hay dato: solo números y longitud)
        ...
        // 9. Verificar código único duplicado (excluyendo el cliente actual)
        if (!empty($_POST['codigo'])) {
            $sql_check_codigo = "SELECT COUNT(*) as count FROM clasif_clientes WHERE codigo = :codigo AND id != :id";
            $stmt_check = $db->prepare($sql_check_codigo);
            $stmt_check->execute(['codigo' => trim($_POST['codigo']), 'id' => $id_cliente]);
            $existe_codigo = $stmt_check->fetch(PDO::FETCH_ASSOC);
            
            if ($existe_codigo['count'] > 0) {
                throw new Exception("El código <strong>" . htmlspecialchars($_POST['codigo']) . "</strong> ya existe en otro cliente.");
            }
        }
        ...
        $datos = [
            ...
            'fechaRegistro' => !empty($_POST['fechaRegistro']) ? $_POST['fechaRegistro'] : $cliente['fechaRegistro'],
            
            // Nuevos campos
            'vigenciapor' => (int)$_POST['vigenciapor'],
            'ResponsableEntidad' => trim($_POST['ResponsableEntidad'] ?? ''),
			'NoCIResp' => !empty($_POST['NoCIResp']) ? trim($_POST['NoCIResp']) : null,
        ...
        $sql_update = "UPDATE clasif_clientes SET 
                      codigo = :codigo, 
                      nombre = :nombre, 
                      direccion = :direccion, 
                      CodReup = :CodReup, 
                      NIT = :NIT, 
                      SucursalCobroLocalidad = :SucursalCobroLocalidad, 
                      NoCtaDeudor = :NoCtaDeudor, 
                      telefono = :telefono, 
                      email = :email, 
                      fechaRegistro = :fechaRegistro, 
                      activo = :activo, 
                      vigenciapor = :vigenciapor, 
                      ResponsableEntidad = :ResponsableEntidad, 
					  NoCIResp = :NoCIResp, 
                      renovac = :renovac, 
                      si_renova_cant = :si_renova_cant, 
        ...
        $stmt_log->execute([
            'EDITAR_CLIENTE',
            'Cliente ' . $datos['nombre'] . ' editado (Código: ' . $datos['codigo'] . ')',
            $_SESSION['usuario_id'],
        ...
        Swal.fire({
            ...
            html: "Los datos del cliente han sido actualizados correctamente.",
            icon: "success",
            background: "var(--win-bg-secondary)",
            color: "var(--win-text-primary)",
            showCancelButton: true,
            confirmButtonText: "<i class=\'fas fa-list\'></i> Volver a Lista",
...
<script src="js/chatbot.js"></script>
    ...
    <style>
        ...
        .win-main-content .form-control, .win-main-content .form-select { background-color: var(--win-bg-tertiary); border: 1px solid var(--win-border-color); color: var(--win-text-primary); border-radius: var(--win-radius-sm); transition: var(--win-transition); }
        ...
        .win-main-content .form-control[readonly] { background-color: rgba(0,0,0,0.2); border-color: var(--win-border-color); opacity: 0.8; cursor: not-allowed; font-weight: bold; }
        ...
        .win-main-content .btn-outline-secondary { color: var(--win-text-secondary); border-color: var(--win-border-color); }
...
<li class="mt-3 mb-2 px-3"><small class="text-muted fw-bold text-uppercase" style="font-size: 10px;">Sistema PDL VISIONES</small></li>
            ...
            <li class="win-nav-item">
                <a href="editar_cliente.php?id=<?php echo htmlspecialchars($cliente['id'] ?? ''); ?>" class="win-nav-link active">
                    <i class="win-nav-icon fas fa-edit"></i>
                    <span class="win-nav-text">Editar Cliente:</span>
...
<div class="mt-4 px-3">
    ...
    <main class="win-main-content <?php echo $sidebar_mini ? 'sidebar-mini' : ''; ?>">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
            <div>
                <h1 class="h2 mb-0" style="color: var(--win-text-primary);"><i class="fas fa-edit me-2" style="color: var(--win-accent);"></i>Editar Cliente</h1>
                <p class="text-muted mb-0">Modifique los datos del cliente - Fecha Operaciones: <strong><?php echo $mes_actual_es . ' ' . date('Y'); ?></strong></p>
            </div>
        ...
        <?php if (isset($error) && !empty($error)): ?>
            <script>document.addEventListener('DOMContentLoaded', ()=> Swal.fire({icon:'error', title:'Error de Validación', html: <?php echo json_encode($error); ?>, background:'#1f1f1f', color:'#fff'}));</script>
        <?php endif; ?>
...
<div class="resumen-cliente animate__animated animate__fadeIn">
    ...
    <div class="resumen-grid">
        <!-- Estado del Contrato (basado en fecha) -->
        <div class="resumen-item">
            <span class="resumen-label">Estado del Contrato:</span>
            <span class="resumen-value <?php echo $estado_contrato_color; ?>" id="resumenEstadoContrato">
        ...
        <div class="resumen-item">
            ...
            <span class="resumen-value" id="resumenCodigo"><?php echo htmlspecialchars($cliente['codigo']); ?></span>
        </div>
        ...
        <div class="resumen-item">
            ...
            <span class="resumen-value" id="resumenFechaInicio"><?php echo date('d/m/Y', strtotime($cliente['fechaRegistro'])); ?></span>
        </div>
        
        <!-- Fecha de Vencimiento -->
        <div class="resumen-item">
            <span class="resumen-label">Fecha Vencimiento:</span>
            <span class="resumen-value" id="resumenFechaFin"><?php echo date('d/m/Y', strtotime($cliente['fechafinalcontrato'])); ?></span>
        </div>
        
        <!-- Estado en BD (activo/inactivo) -->
        <div class="resumen-item">
            <span class="resumen-label">Estado en BD:</span>
            <span class="resumen-value" 
                  style="color: <?php echo $cliente['activo'] == 1 ? '#28a745' : '#dc3545'; ?>;"
                  id="resumenEstadoBD">
        ...
        <div class="resumen-item">
            ...
            <span class="resumen-value 
                <?php 
                    if ($fecha_final_total < $hoy) {
                        echo 'estado-vencido';
                    } elseif (isset($alerta_proximo_vencimiento) && $alerta_proximo_vencimiento) {
                        echo 'estado-vencido';
                    } else {
                        echo 'estado-activo';
                    }
                ...
                <?php if ($fecha_final_total < $hoy): ?>
                    ...
                    <?php 
                        ...
                        if (count($tiempo) > 1) {
                            ...
                            echo implode(", ", $tiempo) . " y " . $ultimo . " vencido" . ($anios_diferencia > 1 || $meses_diferencia > 1 || $dias_restantes > 1 ? 's' : '');
                        } else {
                            echo ($tiempo[0] ?? "0 días") . " vencido" . ($dias_restantes > 1 ? 's' : '');
                        }
                    ?>
                <?php else: ?>
        ...
        <div class="resumen-item">
            ...
            <span class="resumen-value" id="resumenResponsable"><?php echo htmlspecialchars($cliente['ResponsableEntidad'] ?? 'No especificado'); ?></span>
        </div>
        
        <!-- Vigencia (años) -->
        <div class="resumen-item">
            <span class="resumen-label">Vigencia:</span>
            <span class="resumen-value" id="resumenVigencia"><?php echo htmlspecialchars($cliente['vigenciapor'] ?? '0'); ?> año<?php echo ($cliente['vigenciapor'] ?? 1) != 1 ? 's' : ''; ?></span>
        </div>
    </div>
</div>
		...
		<form method="POST" action="" id="clienteForm" class="animate__animated animate__fadeIn" novalidate>
            ...
            <input type="hidden" name="ContratoNo" id="contratoHidden" value="<?php echo htmlspecialchars($cliente['ContratoNo']); ?>">
            
<div class="row">
    <div class="col-lg-8">
        <!-- Información Básica -->
        <div class="card mb-4">
            <div class="card-header fw-bold" style="color: var(--win-text-primary);"><i class="fas fa-info-circle me-2" style="color: var(--win-accent);"></i>Información Básica del Cliente</div>
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-3">
                        <label class="form-label required"><i class="fas fa-barcode me-1" style="color: var(--win-accent);"></i>Código Único</label>
                        <div class="input-group">
                            <input type="text" class="form-control" name="codigo" value="<?php echo htmlspecialchars($cliente['codigo'] ?? ''); ?>" id="codigoInput" required onblur="validarCodigoUnico()" oninput="actualizarResumenCampo('codigo', this.value)">
                        </div>
                    ...
                    <div class="col-md-9">
                        <label class="form-label required"><i class="fas fa-building me-1" style="color: var(--win-accent);"></i>Nombre Empresa, PDL IMDL, MIPYMES, etc.</label>
                        <input type="text" class="form-control" name="nombre" id="nombre" required value="<?php echo htmlspecialchars($cliente['nombre'] ?? ''); ?>">
                        <span class="field-help">Nombre completo de la empresa o cliente</span>
                    </div>
                </div>
                <!-- Responsable de la Entidad y No. CI -->
                <div class="row">
                    <div class="col-md-8 mb-3">
                        <label for="ResponsableEntidad" class="form-label required"><i class="fas fa-user-tie me-1" style="color: var(--win-accent);"></i>Responsable de la Entidad</label>
                        <input type="text" class="form-control <?php echo isset($errores['ResponsableEntidad']) ? 'is-invalid' : ''; ?>" 
                               id="ResponsableEntidad" name="ResponsableEntidad" 
                               value="<?php echo htmlspecialchars($cliente['ResponsableEntidad'] ?? ''); ?>" 
                               maxlength="255" required>
                        <?php if (isset($errores['ResponsableEntidad'])): ?>
                            <div class="invalid-feedback"><?php echo $errores['ResponsableEntidad']; ?></div>
                        <?php endif; ?>
                        <span class="field-help">Nombre completo del responsable</span>
                    </div>
                    
                    <div class="col-md-4 mb-3">
                        <label for="NoCIResp" class="form-label">
                            <i class="fas fa-id-card me-1" style="color: var(--win-accent);"></i>
                            Carné de Identidad (CI)
                        </label>
                        <input type="text" class="form-control <?php echo isset($errores['NoCIResp']) ? 'is-invalid' : ''; ?>" 
                               id="NoCIResp" name="NoCIResp" 
                               value="<?php echo htmlspecialchars($cliente['NoCIResp'] ?? ''); ?>" 
                               maxlength="20" placeholder="Ej: 12345678901">
                        <?php if (isset($errores['NoCIResp'])): ?>
                            <div class="invalid-feedback"><?php echo $errores['NoCIResp']; ?></div>
                        <?php endif; ?>
                        <span class="field-help">Número de C.I. del responsable</span>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 mb-3">
                        <label class="form-label"><i class="fas fa-map-marker-alt me-1" style="color: var(--win-accent);"></i>Dirección</label>
                        <textarea class="form-control" name="direccion" rows="2"><?php echo htmlspecialchars($cliente['direccion'] ?? ''); ?></textarea>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <div class="reup-input-group">
                            <input type="text" class="form-control" name="CodReup" id="reupInput" 
                                   ...
                                   value="<?php echo htmlspecialchars($cliente['CodReup'] ?? ''); ?>" 
                                   maxlength="10" 
        ...
        <!-- Datos Bancarios -->
        <div class="card mb-4">
            <div class="card-header fw-bold" style="color: var(--win-text-primary);"><i class="fas fa-university me-2" style="color: var(--win-accent);"></i>DATOS BANCARIOS</div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label required"><i class="fas fa-file-signature me-1" style="color: var(--win-accent);"></i>Número de Contrato</label>
                        <input type="text" class="form-control" value="<?php echo htmlspecialchars($cliente['ContratoNo'] ?? ''); ?>" disabled id="contratoDisplay">
                        <span class="field-help">Número de contrato CVIS-#### (No editable)</span>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label"><i class="fas fa-code-branch me-1" style="color: var(--win-accent);"></i>Sucursal Bancaria</label>
                        <input type="text" class="form-control" name="SucursalCobroLocalidad" id="sucursalInput" maxlength="8" value="<?php echo htmlspecialchars($cliente['SucursalCobroLocalidad'] ?? ''); ?>" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 8)">
                    ...
                    <div class="col-5 mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" name="NoCtaDeudor" id="cuentaInput" maxlength="16" value="<?php echo htmlspecialchars($cliente['NoCtaDeudor'] ?? ''); ?>" oninput="validarCuentaBancaria()">
                            <!-- No se incluye botón mágico en editar para no sobrescribir código accidentalmente -->
                        </div>
                        
                        <!-- LOGICA SOLICITADA PARA CUENTA BANCARIA -->
                        <small class="text-muted d-block mt-1">
                            <i class="fas fa-info-circle me-1"></i>
                            Formato: <strong>14-16 dígitos numéricos</strong> | 
                            Longitud: <span id="cuenta-length">0</span> dígitos |
                            <span id="cuenta-status" class="ms-2"></span>
                        </small>
                        <div id="cuenta-validation" class="invalid-feedback"></div>
                        <span class="field-help">14 a 16 dígitos.</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <textarea class="form-control" name="observaciones" rows="4"><?php echo htmlspecialchars($cliente['observaciones'] ?? ''); ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        <!-- Contacto -->
        <div class="card mb-4">
            <div class="card-header fw-bold" style="color: var(--win-text-primary);"><i class="fas fa-address-card me-2" style="color: var(--win-accent);"></i>Información de Contacto</div>
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-phone-alt me-1" style="color: var(--win-accent);"></i>Teléfono</label>
                    <input type="tel" class="form-control" name="telefono" id="telefonoInput" value="<?php echo htmlspecialchars($cliente['telefono'] ?? ''); ?>" oninput="validarTelefono(this)">
                    <span class="field-help">Formatos: +53 5 2712861, +53 32415418, 52712861, 0 32415418 (mínimo 8 dígitos)</span>
                </div>
                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-envelope me-1" style="color: var(--win-accent);"></i>Email</label>
                    <input type="email" class="form-control" name="email" id="emailInput" value="<?php echo htmlspecialchars($cliente['email'] ?? ''); ?>">
                    <span class="field-help">Correo electrónico de contacto</span>
                </div>
                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-toggle-on me-1" style="color: var(--win-accent);"></i>Estado del Cliente</label>
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="activo" id="flexSwitchCheckChecked" <?php echo ($cliente['activo'] == 1) ? 'checked' : ''; ?> onchange="actualizarEstado()">
                        <label class="form-check-label fw-bold" for="flexSwitchCheckChecked" id="labelEstado"><?php echo ($cliente['activo'] == 1) ? 'ACTIVO' : 'INACTIVO'; ?></label>
                    </div>
                </div>
        ...
        <div class="card mb-4">
            <div class="card-header fw-bold" style="color: var(--win-text-primary);"><i class="fas fa-calendar-check me-2" style="color: var(--win-accent);"></i>FECHAS CONTRACTUALES</div>
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-calendar-plus me-1" style="color: var(--win-accent);"></i>Fecha de Registro del cliente</label>
                    <input type="date" class="form-control" name="fechaRegistro" id="fechaRegistro" 
           value="<?php echo !empty($cliente['fechaRegistro']) ? $cliente['fechaRegistro'] : date('Y-m-d'); ?>"  onchange="calcularFechas(); actualizarResumenCampo('fechaInicio', this.value)">
                    <span class="field-help">Fecha de registro del cliente</span>
                </div>
                <div class="mb-3">
                    <label class="form-label required"><i class="fas fa-clock me-1" style="color: var(--win-accent);"></i>Vigencia (Años)</label>
                    <input type="number" class="form-control" name="vigenciapor" id="vigenciapor" 
                   value="<?php echo htmlspecialchars($cliente['vigenciapor'] ?? '0'); ?>" 
                   min="1" step="1" required 
                   oninput="this.value=this.value.replace(/[^0-9]/g,''); calcularFechas(); actualizarResumenCampo('vigenciapor', this.value)">
                    <span class="field-help">Vigencia en años enteros</span>
                </div>
                <div class="mb-3">
                    <label class="form-label text-warning"><i class="fas fa-calendar-times me-1" style="color: var(--win-accent);"></i>Fecha de vencimiento del contrato</label>
                    <input type="date" class="form-control" id="fechaVenceDisplay" readonly>
                    <span class="field-help">Calculado: Registro + Vigencia</span>
                </div>
                <hr class="border-secondary my-3">
                <div class="mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="renovac" id="renovacSwitch" onchange="toggleRenovacion()" <?php echo ($cliente['renovac'] == 1) ? 'checked' : ''; ?>>
                        <label class="form-check-label fw-bold" for="renovacSwitch"><i class="fas fa-sync-alt me-1" style="color: var(--win-accent);"></i>¿Renovación de Contrato?</label>
                    </div>
                </div>
                <div id="divRenovacion" class="animate__animated animate__fadeIn" style="<?php echo ($cliente['renovac'] == 1) ? '' : 'display: none;'; ?>">
                    <div class="mb-3 p-3 rounded" style="background: var(--win-bg-tertiary); border: 1px solid var(--win-border-color);">
                        <label class="form-label required"><i class="fas fa-plus-circle me-1" style="color: var(--win-accent);"></i>Cantidad Años Renovación</label>
                        <input type="number" class="form-control mb-2" name="si_renova_cant" id="si_renova_cant" value="<?php echo htmlspecialchars($cliente['si_renova_cant'] ?? '0'); ?>" min="1" step="1" oninput="this.value=this.value.replace(/[^0-9]/g,''); calcularFechas()">
                        <span class="field-help">Años adicionales de renovación</span>
                        <label class="form-label text-success mt-2"><i class="fas fa-calendar-check me-1" style="color: var(--win-accent);"></i>Fecha Final Total</label>
            ...
            <div class="d-flex justify-content-between pt-3 border-top mb-5">
                <p class="text-muted small">* Los campos marcados con asterisco son obligatorios</p>
                <div>
                    <a href="clientes.php" class="btn btn-outline-secondary me-2"><i class="fas fa-times"></i> Cancelar</a>
    ...
    <button class="win-quick-action" onclick="document.getElementById('btnGuardar').click()" title="Guardar Cambios"><i class="fas fa-save"></i></button>
    <?php endif; ?>
    ...
    <script>
        ...
        // Validación en tiempo real para campo NoCtaDeudor
        function validarCuentaBancaria() {
            ...
            cuentaInput.value = cuentaInput.value.substring(0, 16); 
        ...
        function actualizarResumenCampo(campo, valor) {
            switch(campo) {
                case 'codigo':
                    document.getElementById('resumenCodigo').textContent = valor;
                    break;
                case 'ResponsableEntidad':
                    document.getElementById('resumenResponsable').textContent = valor;
                    break;
                case 'vigenciapor':
                    // Actualizar vigencia con plural correcto
                    const vigenciaNum = parseInt(valor) || 0;
                    document.getElementById('resumenVigencia').textContent = 
                        vigenciaNum + " año" + (vigenciaNum !== 1 ? 's' : '');
                    break;
                case 'fechaInicio':
                    if (valor) {
                        let partes = valor.split('-');
                        let fecha = new Date(partes[0], partes[1] - 1, partes[2]);
                        document.getElementById('resumenFechaInicio').textContent = 
        ...
        function formatearTiempo(dias) {
            ...
            let tiempo = [];
            if (years > 0) tiempo.push(years + " año" + (years !== 1 ? 's' : ''));
            if (months > 0) tiempo.push(months + " mes" + (months !== 1 ? 'es' : ''));
            if (days > 0 || (years === 0 && months === 0)) tiempo.push(days + " día" + (days !== 1 ? 's' : ''));
            
            if (tiempo.length > 1) {
                const ultimo = tiempo.pop();
                return tiempo.join(", ") + " y " + ultimo;
            }
            return tiempo[0] || "0 días";
        }

        // Función para actualizar resumen contractual
        function actualizarResumenContrato(fechaFinal) {
            const hoy = new Date();
            const fechaFin = new Date(fechaFinal);
            const resumenEstadoContrato = document.getElementById('resumenEstadoContrato');
            ...
            if (fechaFin < hoy) {
                // Ya venció
                const tiempoVencido = Math.abs(diffDays);
                const tiempoFormateado = formatearTiempo(tiempoVencido);
                
                resumenEstadoContrato.textContent = 'VENCIDO';
                resumenEstadoContrato.className = 'resumen-value estado-vencido';
                
                resumenTiempo.innerHTML = `<i class="fas fa-calendar-times me-1"></i>${tiempoFormateado} vencido${tiempoVencido !== 1 ? 's' : ''}`;
                resumenTiempo.className = 'resumen-value estado-vencido';
                
            } else {
                // Aún no vence
                const tiempoFormateado = formatearTiempo(diffDays);
                const alertaProximo = (diffDays <= 7);
                
                resumenEstadoContrato.textContent = 'ACTIVO';
                resumenEstadoContrato.className = 'resumen-value estado-activo';
                
                if (alertaProximo) {
                    resumenTiempo.innerHTML = `<i class="fas fa-clock me-1"></i>${diffDays} día${diffDays !== 1 ? 's' : ''} <small class="text-muted ms-1">(${tiempoFormateado})</small>`;
                    resumenTiempo.className = 'resumen-value estado-vencido';
                } else {
                    resumenTiempo.innerHTML = `<i class="fas fa-calendar-check me-1"></i>${tiempoFormateado}`;
                    resumenTiempo.className = 'resumen-value estado-activo';
                }
            }
            
            // Actualizar fecha en resumen
            const parts = fechaFinal.split('-');
            document.getElementById('resumenFechaFin').textContent = `${parts[2]}/${parts[1]}/${parts[0]}`;
        }

        // Actualizar estado en BD
        function actualizarEstadoBD() {
            const estadoCheck = document.getElementById('flexSwitchCheckChecked');
            const resumenEstadoBD = document.getElementById('resumenEstadoBD');
            
            if (estadoCheck.checked) {
                resumenEstadoBD.innerHTML = 'ACTIVO (1) <i class="fas fa-check-circle ms-1"></i>';
                resumenEstadoBD.style.color = '#28a745';
            } else {
                resumenEstadoBD.innerHTML = 'INACTIVO (0) <i class="fas fa-times-circle ms-1"></i>';
                resumenEstadoBD.style.color = '#dc3545';
            }
        }

        // Actualizar estado del cliente (BD)
        function actualizarEstado() {
            ...
            // Actualizar resumen de estado en BD
            actualizarEstadoBD();
        }
        ...
        function calcularFechas() {
            ...
            let fFinalStr = fFinal.toISOString().split('T')[0];
            document.getElementById('fechafinalcontratoDisplay').value = fFinalStr;
            
            // Actualizar resumen contractual
            actualizarResumenContrato(fFinalStr);
            
            // Mostrar alerta si está vencido
            let hoy = new Date(); 
            ...
            if(fFinal < hoy) {
                ...
                // Si está vencido, sugerir cambiar estado en BD a inactivo
                if (document.getElementById('flexSwitchCheckChecked').checked) {
                    Swal.fire({
                        title: '⚠️ Contrato Vencido',
                        text: 'El contrato está vencido. ¿Desea cambiar el estado del cliente a INACTIVO?',
                        icon: 'warning',
                        showCancelButton: true,
                    confirmButtonText: '<i class="fas fa-check-circle"></i> Sí, Cambiar a INACTIVO',
                    cancelButtonText: '<i class="fas fa-times"></i> No, Mantener Activo',
                    }).then((result) => {
                        if (result.isConfirmed) {
        ...
        document.getElementById('clienteForm').addEventListener('submit', function(e) {
            ...
            // Check vencimiento
            let alertVencido = document.getElementById('alertVencido');
            if (alertVencido.style.display === 'block') {
                vencidoWarning = true;
            }
            ...
            } else if (vencidoWarning) {
                Swal.fire({
                    title: '⚠️ CONTRATO VENCIDO',
                    html: `
                        <div style="text-align: left;">
                            <p><strong>¿Desea guardar un contrato vencido?</strong></p>
                            <p class="text-warning"><i class="fas fa-exclamation-triangle"></i> Este contrato estará vencido según la fecha calculada.</p>
                        </div>
                    `,
                    icon: 'warning',
                    showCancelButton: true,
        ...
        function confirmarEdicion() {
            const empresa = document.getElementById('nombre').value;
            const responsable = document.getElementById('ResponsableEntidad').value;
            const fechaFinRaw = document.getElementById('fechafinalcontratoDisplay').value;
            
            let fechaFinBonita = "---";
            if(fechaFinRaw) {
                const [year, month, day] = fechaFinRaw.split('-');
                fechaFinBonita = `${day}/${month}/${year}`;
            }
            ...
            Swal.fire({
                ...
                html: `
                    <div class="text-start">
                        <p><strong>Empresa/Cliente:</strong> ${empresa}</p>
                        <p><strong>Responsable Entidad (Director):</strong> ${responsable}</p>
                        <p><strong>Fecha Vencimiento Contractual:</strong> <span class="text-warning fw-bold">${fechaFinBonita}</span></p>
                    </div>
                `,
                icon: 'question', 
                showCancelButton: true, 
                confirmButtonText: '<i class="fas fa-save me-2"></i> Sí, Guardar', 
                cancelButtonText: '<i class="fas fa-save me-2"></i> Cancelar',
                background: '#1f1f1f', 
                color:'#fff'
            }).then((r) => { 
                if(r.isConfirmed) document.getElementById('clienteForm').submit(); 
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('cuenta-length').textContent = document.getElementById('cuentaInput').value.length;
            ...
            // Inicializar estado BD y resumen contractual
            actualizarEstadoBD();
            
            const fechaFinal = document.getElementById('fechafinalcontratoDisplay').value;
            if (fechaFinal) {
                actualizarResumenContrato(fechaFinal);
            }
        });
    </script>
    <?php
