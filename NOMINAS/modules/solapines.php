...
if (!isset($_SESSION['usuario_id']) && !isset($_SESSION['logged_in'])) {
    ...
    die("Error: No autorizado. Inicie sesión nuevamente.");
}
...
if (isset($_GET['exportar_todos'])) {
    ...
    $stmt_todos = $pdo->query("SELECT id, nombres, primer_apellido, segundo_apellido FROM trabajadores");
    $trabajadores = $stmt_todos->fetchAll(PDO::FETCH_ASSOC);

    if (empty($trabajadores)) {
        die("No se encontraron trabajadores activos para exportar.");
    }
